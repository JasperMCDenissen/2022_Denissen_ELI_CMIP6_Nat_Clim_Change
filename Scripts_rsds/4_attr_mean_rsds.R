...
dcorr_rsds_all.array <- av_tas.array <-
...
count_all <- 1
for(i in 1:11){
  dcorr_rsds_all.array[,,count_all:(count_all+11)] <- dcorr_rsds.list[[i]]
...
test <- array(NaN,c(180,90,11*12)); test[1:90,,] <- dcorr_rsds_all.array[91:180,,]; test[91:180,,] <- dcorr_rsds_all.array[1:90,,]; dcorr_rsds_all.array <- test; dcorr_rsds_all.array <- -1*dcorr_rsds_all.array
...
for(x in 1:180){
  for(y in 1:90){
    if(sum(!is.na(dcorr_rsds_all.array[x,y,])) == 11*12){ # check if all the models have a value there
      mask_obs[x,y] <- 1
    }
  }
}

mmmean_dcorr_rsds_per_10yr <- mmmean_tas_per_10yr <- mmmean_rsds_per_10yr <-
    ...
    for(y in 1:90){
      if(!is.na(models_with_full_timeseries[x,y])){
        if(models_with_full_timeseries[x,y] > 4){
          mmmean_dcorr_rsds_per_10yr[x,y,i] <- mean(dcorr_rsds_all.array[x,y,(index_per_10yr+i)],na.rm=T)
...
mmmean_kendall_dcorr_rsds <- mmmean_kendall_tas <- mmmean_kendall_rsds <- mmmean_kendall_hfls <-
...
for(x in 1:180){
  for(y in 1:90){
    if(sum(!is.na(mmmean_dcorr_rsds_per_10yr[x,y,])) == 12){
      sum_blocks[x,y] <- sum(!is.na(mmmean_dcorr_rsds_per_10yr[x,y,]))
...
vars <- c("rsds","mrso","hfls","lai","ar","adj. R2 < 0.5")
...
for(x in 1:180){
  for(y in 1:90){
    if(sum(!is.na(mmmean_dcorr_rsds_per_10yr[x,y,])) == 12 & sum(!is.na(mmmean_lai_per_10yr[x,y,])) == 12 & round(sd(mmmean_lai_per_10yr[x,y,]),4) != 0 & round(sd(mmmean_mrso_per_10yr[x,y,]),4) != 0){ # there should be variability in mrso and lai, otherwise there is no explanatory power in their respective time series
      relimp_variables.list <- list()
      rsq.vec <- c()
      # dredge: automated model selection
      lm_dcorr_rsds.df <- lm(mmmean_dcorr_rsds_per_10yr[x,y,] ~
...
bar_dom_var_dredge_relimp <- ggplot(count_dom_var_dredge_relimp_reg.df, aes(x = reg_factor, y = sign_area, fill = var_factor, label=round(sign_area,2))) +
  ...
  scale_fill_manual("",labels = c("Shorwave incoming radiation","Soil moisture","Terrestrial evaporation","Leaf Area Index","Aridity Index"),
...
bar_dom_var_dredge_relimp_rsds_per_reg <- ggplot(count_dom_var_dredge_relimp_reg.df[which(count_dom_var_dredge_relimp_reg.df$var == 'rsds'),], aes(x = reg_factor, y = sign_area, fill = var_factor, label=round(sign_area,0))) +
...
bar_dom_var_dredge_relimp_rsds_per_reg
bar_dom_var_dredge_relimp_rsds_per_reg <- ggplotGrob(bar_dom_var_dredge_relimp_rsds_per_reg)
plot_bar_dom_var_dredge_relimp_rsds_per_reg <- bar_dom_var_dredge_relimp_rsds_per_reg
...
a <- ggplot(dom_var_ts.df, aes(x=lon,y=lat,fill=cuts_dom_var_dredge_relimp)) +
  ...
  scale_fill_manual("Dominant variable",
                    values = cols_dom_var,
                    labels = c("Shorwave incoming radiation","Soil moisture","Terrestrial evaporation","Leaf Area Index","Aridity Index",expression("adjusted R"^2*" < 0.5")),
                    drop = F) +
...
gt1 <- gtable_add_grob(gt1, plot_bar_dom_var_dredge_relimp_rsds_per_reg, t = 26, l = 3, b = 18, r = 8)
...
options(na.action = "na.fail")
vars <- c("rsds","mrso","hfls","lai","ar","adj. R2 < 0.7")
# dredge + calc.relimp()
# https://onlinelibrary.wiley.com/doi/full/10.1111/gcb.15385
dom_var_dredge_relimp <- array(NaN,c(180,90,2))
dom_var_dredge_relimp_all <- array(0,c(180,90,5))
...
a <- ggplot(dom_var_ts.df, aes(x=lon,y=lat,fill=cuts_dom_var_dredge_relimp)) + 
  ...
  scale_fill_manual("Dominant variable",
                    values = cols_dom_var, 
                    labels = c("Shorwave incoming radiation","Soil moisture","Terrestrial evaporation","Leaf Area Index","Aridity Index",expression("adjusted R"^2*" < 0.7")),
                    drop = F) +
  scale_x_continuous("longitude",
                     limits=c(-180,180),
                     expand=c(0,0)) +
...
options(na.action = "na.fail")
vars <- c("rsds","mrso","hfls","lai","ar","adj. R2 < 0.3")
# dredge + calc.relimp()
# https://onlinelibrary.wiley.com/doi/full/10.1111/gcb.15385
dom_var_dredge_relimp <- array(NaN,c(180,90,2))
dom_var_dredge_relimp_all <- array(0,c(180,90,5))
...
a <- ggplot(dom_var_ts.df, aes(x=lon,y=lat,fill=cuts_dom_var_dredge_relimp)) + 
  ...
  scale_fill_manual("Dominant variable",
                    values = cols_dom_var, 
                    labels = c("Shorwave incoming radiation","Soil moisture","Terrestrial evaporation","Leaf Area Index","Aridity Index",expression("adjusted R"^2*" < 0.3")),
                    drop = F) +
  scale_x_continuous("longitude",
                     limits=c(-180,180),
                     expand=c(0,0)) +
