...
for(i in 1:length(cmip6_data.df$source_id)){
  ...
  for(t in seq(1,1440,120)){ # so the loop moves 10 years and in the loop we read in 10 years of data.
    ...
    netrad_10yr <- clusterR(rsds_10yr, fun = function(a,b){return(a+b)},
                            args = list(b = rsus_10yr), progress = 'text')
    ...
    stack_anom_tas_hfls_mrso_wrm <- stack(anom_tas_wrm_10yr, anom_hfls_wrm_10yr, anom_mrso_wrm_10yr)
    stack_anom_mrso_hfls_mrso_wrm <- stack(anom_mrso_wrm_10yr, anom_hfls_wrm_10yr, anom_tas_wrm_10yr)
    pcorr_rgy_veg_10yr <- clusterR(stack_anom_tas_hfls_mrso_wrm, calc, args = list(pcorr_fun_10yr), progress = 'text')
    ...
    for(s in 1:12){
      stack_anom_tas_hfls_wrm_moy <- stack(anom_tas_wrm_10yr[[which(moy == s)]], anom_hfls_wrm_10yr[[which(moy == s)]])
      stack_anom_mrso_hfls_wrm_moy <- stack(anom_mrso_wrm_10yr[[which(moy == s)]], anom_hfls_wrm_10yr[[which(moy == s)]])
      corr_rgy_veg_moy <- clusterR(stack_anom_tas_hfls_wrm_moy, calc, args = list(corr_fun_seascycle), progress = 'text')
  ...
  save(dcorr.list, 
       ...
       sd_anom_mrso.list,
       sd_anom_hfls.list,
       file = "testdir/202112_dcorr_cmip6_10yr_no_evspsblveg_hurs.RData")
}
