...
dcorr_netrad.list <- list(dcorr_netrad.list[[10]], dcorr_netrad.list[[9]], dcorr_netrad.list[[11]], dcorr_netrad.list[[2]],
...
bar_treeFractrends_rev <- ggplot(dcorr_treeFrac.df, aes(y = label, x = mmmean_kendall_dcorr, fill = label)) +
  ...
  scale_fill_manual(values = c(brewer.pal(n=9, "YlOrRd")[1], brewer.pal(n=5, "Greens")[2:5]),
                    drop = F) +
...
corr_rgy_veg.list <- list(corr_rgy_veg.list[[1]], corr_rgy_veg.list[[2]], corr_rgy_veg.list[[3]], corr_rgy_veg.list[[4]],
                   corr_rgy_veg.list[[5]], corr_rgy_veg.list[[6]], corr_rgy_veg.list[[7]], corr_rgy_veg.list[[9]],
                   corr_rgy_veg.list[[10]], corr_rgy_veg.list[[11]], corr_rgy_veg.list[[12]])
corr_wtr_veg.list <- list(corr_wtr_veg.list[[1]], corr_wtr_veg.list[[2]], corr_wtr_veg.list[[3]], corr_wtr_veg.list[[4]],
                   corr_wtr_veg.list[[5]], corr_wtr_veg.list[[6]], corr_wtr_veg.list[[7]], corr_wtr_veg.list[[9]],
                   corr_wtr_veg.list[[10]], corr_wtr_veg.list[[11]], corr_wtr_veg.list[[12]])
dcorr_netrad.list <- list(dcorr_netrad.list[[1]], dcorr_netrad.list[[2]], dcorr_netrad.list[[3]], dcorr_netrad.list[[4]],
                          dcorr_netrad.list[[5]], dcorr_netrad.list[[6]], dcorr_netrad.list[[7]], dcorr_netrad.list[[9]],
                          dcorr_netrad.list[[10]], dcorr_netrad.list[[11]], dcorr_netrad.list[[12]])
corr_rgy_veg_netrad.list <- list(corr_rgy_veg_netrad.list[[1]], corr_rgy_veg_netrad.list[[2]], corr_rgy_veg_netrad.list[[3]], corr_rgy_veg_netrad.list[[4]],
                                 corr_rgy_veg_netrad.list[[5]], corr_rgy_veg_netrad.list[[6]], corr_rgy_veg_netrad.list[[7]], corr_rgy_veg_netrad.list[[9]],
                                 corr_rgy_veg_netrad.list[[10]], corr_rgy_veg_netrad.list[[11]], corr_rgy_veg_netrad.list[[12]])
corr_wtr_veg_netrad.list <- list(corr_wtr_veg_netrad.list[[1]], corr_wtr_veg_netrad.list[[2]], corr_wtr_veg_netrad.list[[3]], corr_wtr_veg_netrad.list[[4]],
                                 corr_wtr_veg_netrad.list[[5]], corr_wtr_veg_netrad.list[[6]], corr_wtr_veg_netrad.list[[7]], corr_wtr_veg_netrad.list[[9]],
                                 corr_wtr_veg_netrad.list[[10]], corr_wtr_veg_netrad.list[[11]], corr_wtr_veg_netrad.list[[12]])
...
kendall_dcorr <-
  kendall_dcorr_netrad <-
  array(NaN,c(180,90,11,2)) # 7: models, 2: slope (all) & p.value (all)
count_i <- 1
...
dcorr.df <- data.frame("ensemble_mean_dcorr_all" = ensemble_mean_dcorr_all_vec,
                       ...
                       # "ensemble_mean_dcorr_all_mask_cmip6" = ensemble_mean_dcorr_all_vec_mask_cmip6,
...
mmmean_dcorr_per_10yr <- mmmean_corr_rgy_veg_per_10yr <- mmmean_corr_wtr_veg_per_10yr <- mmmean_tas_per_10yr <- 
...
area_per_10yr.df <- setNames(data.frame(matrix(ncol = 3, nrow = 0)),
                             c("year","area","region"))
...
dbar <- plot_discrete_cbar(breaks = c(myvalues_ensemble_mean_dcorr[1:4],0,myvalues_ensemble_mean_dcorr[7:10]),
                           ...
                           legend_title = "mean Ecosystem Limitation Index",
                           spacing = "constant",
                           font_size = 6,
                           spacing_scaling = 2,
                           width = .2,
...
point_mmmean_dcorr_per_10yr_reg <- ggplot(area_per_10yr.df, aes(x = year, y = area, col = reg)) +
  geom_line(size = 1) +
  scale_x_continuous("",
                     breaks = year_id[seq(1,11,2)], expand = c(0,0)) +
  scale_y_continuous(expression(paste("water-limited land area-%")), expand = c(0,0)) +
...
ggsave("testdir/SFig11.png", plot = point_mmmean_dcorr_per_10yr_reg, width = 11*1.25, height = 5*1.25, units = "in")
...
dbar <- plot_discrete_cbar(breaks = c(myvalues_ensemble_mean_dcorr[1:4],0,myvalues_ensemble_mean_dcorr[7:10]),
                           colors = c(dcorrcol[1:4],dcorrcol[6:9]),
                           legend_title = expression("mean Ecosystem Limitation Index (with R"[n]*")"),
                           spacing = "constant",
                           font_size = 6,
                           spacing_scaling = 2,
                           width = .2,
...
sd_kendall_dcorr <- array(NaN,c(180,90))
for(x in 1:180){
  for(y in 1:90){
    if(models_with_full_timeseries[x,y] > 4){
      sd_kendall_dcorr[x,y] <- sd(kendall_dcorr[x,y,,1], na.rm = T)
    }
  }
}
...
for(x in 1:180){
  for(y in 1:90){
    ...
    sd_kendall_dcorr_vec[count] <- sd_kendall_dcorr[x,y]
    count <- count + 1
  }
}
...
mmtrends.df <- data.frame('trend_dcorr' = c(mmmean_kendall_dcorr_vec),
                          ...
                          'sd_kendall_dcorr' = c(sd_kendall_dcorr_vec),
                          "lon" = rep(lon, each = 90),
                          "lat" = rep(lat, 180))
...
dbar_netrad <- plot_discrete_cbar(myvalues_trnd_dcorr,
                                  colors = cols_trnd_dcorr,
                                  legend_title = expression("ELI"[R[n]]*" trend (-/10yr)"),
                                  spacing = "constant",
                                  font_size = 6,
                                  spacing_scaling = 2,
                                  width = .2,
...
dbar_corr_rgy_veg <- plot_discrete_cbar(myvalues_trnd_corr_rgy_veg,
                                        ...
                                        legend_title = expression("cor(T"[a]*"',ET') trend (-/10yr)"),
                                        spacing = "constant",
                                        font_size = 6,
                                        spacing_scaling = 2,
                                        width = .2,
...
mmtrends.df$cuts_sd_kendall_dcorr <- cut(mmtrends.df$sd_kendall_dcorr, myvalues_trnd_sd_dcorr, include.lowest = T)
...
plots1 <- grid.arrange(plot_mean_kendall_tas, plot_mean_kendall_mrso, plot_mean_kendall_hfls, ncol = 3, widths = c(1/3,1/3,1/3))
...
ggsave("testdir/SFig10.png", plot = plots3, width = 11*1.9, height = 5.5*1.9, units = "in")

# the mean corr_rgy_veg plot
count_mmtrends.df <- data.frame("area" = c(100*c(sum(area.array[which(mmmean_kendall_corr_rgy_veg[,,1] > 0)])/total_land_area, sum(area.array[which(mmmean_kendall_corr_rgy_veg[,,1] < 0)])/total_land_area),
                                           100*c(sum(area.array[which(mmmean_kendall_corr_rgy_veg[,,1] > 0 & mmmean_kendall_corr_rgy_veg[,,2] < .05)])/total_land_area, sum(area.array[which(mmmean_kendall_corr_rgy_veg[,,1] < 0 & mmmean_kendall_corr_rgy_veg[,,2] < .05)])/total_land_area),
                                           100*c(sum(area.array[which(mmmean_kendall_corr_rgy_veg[,,1] > 0 & mmmean_kendall_corr_rgy_veg[,,2] > .05)])/total_land_area, sum(area.array[which(mmmean_kendall_corr_rgy_veg[,,1] < 0 & mmmean_kendall_corr_rgy_veg[,,2] > .05)])/total_land_area)),
...
ggsave("testdir/SFig9.png", plot = plots1, width = 9*1.2, height = 7*2*1.2, units = "in")


# the mean sd_kendall_dcorr plot
f <- ggplot(mmtrends.df[which(!is.na(mmtrends.df$trend_dcorr)),], aes(x=lon,y=lat,fill=cuts_sd_kendall_dcorr)) +
  geom_tile() +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
...
for(x in 1:180){
  for(y in 1:90){
    if(!is.na(models_with_full_timeseries[x,y])){
      if(models_with_full_timeseries[x,y] > 4){
        agreement.array[x,y] <- length(which(sign(kendall_dcorr[x,y,,1]) == sign(mmmean_kendall_dcorr[x,y,1]))) / sum(!is.na(kendall_dcorr[x,y,,1]))*100
      }
    }
  }
}
...
ggsave("testdir/SFig8.png", plot = plots, width = 9, height = 14, units = "in")

# make spatial plots per model for the dcorr trend
kendall_dcorr_per_model.df <- setNames(data.frame(matrix(ncol = 4, nrow = 0)),
                                       c("trend_dcorr","lon","lat","source_id"))
for(i in 1:11){
  for(x in 1:180){
    for(y in 1:90){
      if(!is.na(kendall_dcorr[x,y,i,1])){
        kendall_dcorr_per_model.df <- rbind(kendall_dcorr_per_model.df,
                                            data.frame("trend_dcorr" = kendall_dcorr[x,y,i,1],
                                                       "p" = kendall_dcorr[x,y,i,2],
                                                       "lon" = lon[x],
                                                       "lat" = lat[y],
...
kendall_dcorr_per_model.df$cuts_trend_dcorr <- cut(kendall_dcorr_per_model.df$trend_dcorr, myvalues_trnd_dcorr, include.lowest = T)

sign_kendall_dcorr_per_model.df <- kendall_dcorr_per_model.df[,c(3,4,5)][which(kendall_dcorr_per_model.df$p < .05),]

# calculate total_land_area_sourceid
total_land_area_sourceid <- c()
for(i in 1:11){
  total_land_area_sourceid[i] <- sum(area.array[which(!is.na(kendall_dcorr[,,i,1]))])
}



# the dcorr trend per source_id
count_kendall_dcorr_per_model.df <- data.frame("area" = c(100*c(sum(area.array[which(kendall_dcorr[,,1,1] > 0)])/total_land_area_sourceid[1], sum(area.array[which(kendall_dcorr[,,1,1] < 0)])/total_land_area_sourceid[1],
                                                                sum(area.array[which(kendall_dcorr[,,1,1] > 0 & kendall_dcorr[,,1,2] < .05)])/total_land_area_sourceid[1], sum(area.array[which(kendall_dcorr[,,1,1] < 0 & kendall_dcorr[,,1,2] < .05)])/total_land_area_sourceid[1],
                                                                sum(area.array[which(kendall_dcorr[,,1,1] > 0 & kendall_dcorr[,,1,2] > .05)])/total_land_area_sourceid[1], sum(area.array[which(kendall_dcorr[,,1,1] < 0 & kendall_dcorr[,,1,2] > .05)])/total_land_area_sourceid[1]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,2,1] > 0)])/total_land_area_sourceid[2], sum(area.array[which(kendall_dcorr[,,2,1] < 0)])/total_land_area_sourceid[2],
                                                                sum(area.array[which(kendall_dcorr[,,2,1] > 0 & kendall_dcorr[,,2,2] < .05)])/total_land_area_sourceid[2], sum(area.array[which(kendall_dcorr[,,2,1] < 0 & kendall_dcorr[,,2,2] < .05)])/total_land_area_sourceid[2],
                                                                sum(area.array[which(kendall_dcorr[,,2,1] > 0 & kendall_dcorr[,,2,2] > .05)])/total_land_area_sourceid[2], sum(area.array[which(kendall_dcorr[,,2,1] < 0 & kendall_dcorr[,,2,2] > .05)])/total_land_area_sourceid[2]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,3,1] > 0)])/total_land_area_sourceid[3], sum(area.array[which(kendall_dcorr[,,3,1] < 0)])/total_land_area_sourceid[3],
                                                                sum(area.array[which(kendall_dcorr[,,3,1] > 0 & kendall_dcorr[,,3,2] < .05)])/total_land_area_sourceid[3], sum(area.array[which(kendall_dcorr[,,3,1] < 0 & kendall_dcorr[,,3,2] < .05)])/total_land_area_sourceid[3],
                                                                sum(area.array[which(kendall_dcorr[,,3,1] > 0 & kendall_dcorr[,,3,2] > .05)])/total_land_area_sourceid[3], sum(area.array[which(kendall_dcorr[,,3,1] < 0 & kendall_dcorr[,,3,2] > .05)])/total_land_area_sourceid[3]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,4,1] > 0)])/total_land_area_sourceid[4], sum(area.array[which(kendall_dcorr[,,4,1] < 0)])/total_land_area_sourceid[4],
                                                                sum(area.array[which(kendall_dcorr[,,4,1] > 0 & kendall_dcorr[,,4,2] < .05)])/total_land_area_sourceid[4], sum(area.array[which(kendall_dcorr[,,4,1] < 0 & kendall_dcorr[,,4,2] < .05)])/total_land_area_sourceid[4],
                                                                sum(area.array[which(kendall_dcorr[,,4,1] > 0 & kendall_dcorr[,,4,2] > .05)])/total_land_area_sourceid[4], sum(area.array[which(kendall_dcorr[,,4,1] < 0 & kendall_dcorr[,,4,2] > .05)])/total_land_area_sourceid[4]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,5,1] > 0)])/total_land_area_sourceid[5], sum(area.array[which(kendall_dcorr[,,5,1] < 0)])/total_land_area_sourceid[5],
                                                                sum(area.array[which(kendall_dcorr[,,5,1] > 0 & kendall_dcorr[,,5,2] < .05)])/total_land_area_sourceid[5], sum(area.array[which(kendall_dcorr[,,5,1] < 0 & kendall_dcorr[,,5,2] < .05)])/total_land_area_sourceid[5],
                                                                sum(area.array[which(kendall_dcorr[,,5,1] > 0 & kendall_dcorr[,,5,2] > .05)])/total_land_area_sourceid[5], sum(area.array[which(kendall_dcorr[,,5,1] < 0 & kendall_dcorr[,,5,2] > .05)])/total_land_area_sourceid[5]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,6,1] > 0)])/total_land_area_sourceid[6], sum(area.array[which(kendall_dcorr[,,6,1] < 0)])/total_land_area_sourceid[6],
                                                                sum(area.array[which(kendall_dcorr[,,6,1] > 0 & kendall_dcorr[,,6,2] < .05)])/total_land_area_sourceid[6], sum(area.array[which(kendall_dcorr[,,6,1] < 0 & kendall_dcorr[,,6,2] < .05)])/total_land_area_sourceid[6],
                                                                sum(area.array[which(kendall_dcorr[,,6,1] > 0 & kendall_dcorr[,,6,2] > .05)])/total_land_area_sourceid[6], sum(area.array[which(kendall_dcorr[,,6,1] < 0 & kendall_dcorr[,,6,2] > .05)])/total_land_area_sourceid[6]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,7,1] > 0)])/total_land_area_sourceid[7], sum(area.array[which(kendall_dcorr[,,7,1] < 0)])/total_land_area_sourceid[7],
                                                                sum(area.array[which(kendall_dcorr[,,7,1] > 0 & kendall_dcorr[,,7,2] < .05)])/total_land_area_sourceid[7], sum(area.array[which(kendall_dcorr[,,7,1] < 0 & kendall_dcorr[,,7,2] < .05)])/total_land_area_sourceid[7],
                                                                sum(area.array[which(kendall_dcorr[,,7,1] > 0 & kendall_dcorr[,,7,2] > .05)])/total_land_area_sourceid[7], sum(area.array[which(kendall_dcorr[,,7,1] < 0 & kendall_dcorr[,,7,2] > .05)])/total_land_area_sourceid[7]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,8,1] > 0)])/total_land_area_sourceid[8], sum(area.array[which(kendall_dcorr[,,8,1] < 0)])/total_land_area_sourceid[8],
                                                                sum(area.array[which(kendall_dcorr[,,8,1] > 0 & kendall_dcorr[,,8,2] < .05)])/total_land_area_sourceid[8], sum(area.array[which(kendall_dcorr[,,8,1] < 0 & kendall_dcorr[,,8,2] < .05)])/total_land_area_sourceid[8],
                                                                sum(area.array[which(kendall_dcorr[,,8,1] > 0 & kendall_dcorr[,,8,2] > .05)])/total_land_area_sourceid[8], sum(area.array[which(kendall_dcorr[,,8,1] < 0 & kendall_dcorr[,,8,2] > .05)])/total_land_area_sourceid[8]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,9,1] > 0)])/total_land_area_sourceid[9], sum(area.array[which(kendall_dcorr[,,9,1] < 0)])/total_land_area_sourceid[9],
                                                                sum(area.array[which(kendall_dcorr[,,9,1] > 0 & kendall_dcorr[,,9,2] < .05)])/total_land_area_sourceid[9], sum(area.array[which(kendall_dcorr[,,9,1] < 0 & kendall_dcorr[,,9,2] < .05)])/total_land_area_sourceid[9],
                                                                sum(area.array[which(kendall_dcorr[,,9,1] > 0 & kendall_dcorr[,,9,2] > .05)])/total_land_area_sourceid[9], sum(area.array[which(kendall_dcorr[,,9,1] < 0 & kendall_dcorr[,,9,2] > .05)])/total_land_area_sourceid[9]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,10,1] > 0)])/total_land_area_sourceid[10], sum(area.array[which(kendall_dcorr[,,10,1] < 0)])/total_land_area_sourceid[10],
                                                                sum(area.array[which(kendall_dcorr[,,10,1] > 0 & kendall_dcorr[,,10,2] < .05)])/total_land_area_sourceid[10], sum(area.array[which(kendall_dcorr[,,10,1] < 0 & kendall_dcorr[,,10,2] < .05)])/total_land_area_sourceid[10],
                                                                sum(area.array[which(kendall_dcorr[,,10,1] > 0 & kendall_dcorr[,,10,2] > .05)])/total_land_area_sourceid[10], sum(area.array[which(kendall_dcorr[,,10,1] < 0 & kendall_dcorr[,,10,2] > .05)])/total_land_area_sourceid[10]),
                                                          100*c(sum(area.array[which(kendall_dcorr[,,11,1] > 0)])/total_land_area_sourceid[11], sum(area.array[which(kendall_dcorr[,,11,1] < 0)])/total_land_area_sourceid[11],
                                                                sum(area.array[which(kendall_dcorr[,,11,1] > 0 & kendall_dcorr[,,11,2] < .05)])/total_land_area_sourceid[11], sum(area.array[which(kendall_dcorr[,,11,1] < 0 & kendall_dcorr[,,11,2] < .05)])/total_land_area_sourceid[11],
                                                                sum(area.array[which(kendall_dcorr[,,11,1] > 0 & kendall_dcorr[,,11,2] > .05)])/total_land_area_sourceid[11], sum(area.array[which(kendall_dcorr[,,11,1] < 0 & kendall_dcorr[,,11,2] > .05)])/total_land_area_sourceid[11])),
...
count_kendall_dcorr_per_model.df$trend_sign <- paste0(count_kendall_dcorr_per_model.df$trend, "_",count_kendall_dcorr_per_model.df$sign)

bar_kendall_dcorr_per_model1 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[1] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[1] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model1
bar_kendall_dcorr_per_model1 <- ggplotGrob(bar_kendall_dcorr_per_model1)

f1 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[1]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[1]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt1 <- gtable_add_grob(gt1, bar_kendall_dcorr_per_model1, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt1)


bar_kendall_dcorr_per_model2 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[2] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[2] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model2
bar_kendall_dcorr_per_model2 <- ggplotGrob(bar_kendall_dcorr_per_model2)

f2 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[2]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[2]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt2 <- gtable_add_grob(gt2, bar_kendall_dcorr_per_model2, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt2)

bar_kendall_dcorr_per_model3 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[3] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[3] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model3
bar_kendall_dcorr_per_model3 <- ggplotGrob(bar_kendall_dcorr_per_model3)

f3 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[3]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[3]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt3 <- gtable_add_grob(gt3, bar_kendall_dcorr_per_model3, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt3)

bar_kendall_dcorr_per_model4 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[4] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[4] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model4
bar_kendall_dcorr_per_model4 <- ggplotGrob(bar_kendall_dcorr_per_model4)

f4 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[4]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[4]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt4 <- gtable_add_grob(gt4, bar_kendall_dcorr_per_model4, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt4)

bar_kendall_dcorr_per_model5 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[5] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[5] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model5
bar_kendall_dcorr_per_model5 <- ggplotGrob(bar_kendall_dcorr_per_model5)

f5 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[5]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[5]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt5 <- gtable_add_grob(gt5, bar_kendall_dcorr_per_model5, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt5)

bar_kendall_dcorr_per_model6 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[6] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[6] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model6
bar_kendall_dcorr_per_model6 <- ggplotGrob(bar_kendall_dcorr_per_model6)

f6 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[6]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[6]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt6 <- gtable_add_grob(gt6, bar_kendall_dcorr_per_model6, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt6)

bar_kendall_dcorr_per_model7 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[7] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[7] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model7
bar_kendall_dcorr_per_model7 <- ggplotGrob(bar_kendall_dcorr_per_model7)

f7 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[7]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[7]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt7 <- gtable_add_grob(gt7, bar_kendall_dcorr_per_model7, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt7)

bar_kendall_dcorr_per_model8 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[8] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[8] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model8
bar_kendall_dcorr_per_model8 <- ggplotGrob(bar_kendall_dcorr_per_model8)

f8 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[8]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[8]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt8 <- gtable_add_grob(gt8, bar_kendall_dcorr_per_model8, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt8)

bar_kendall_dcorr_per_model9 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[9] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[9] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model9
bar_kendall_dcorr_per_model9 <- ggplotGrob(bar_kendall_dcorr_per_model9)

f9 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[9]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[9]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt9 <- gtable_add_grob(gt9, bar_kendall_dcorr_per_model9, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt9)

bar_kendall_dcorr_per_model10 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[10] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[10] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model10
bar_kendall_dcorr_per_model10 <- ggplotGrob(bar_kendall_dcorr_per_model10)

f10 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[10]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[10]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt10 <- gtable_add_grob(gt10, bar_kendall_dcorr_per_model10, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt10)

bar_kendall_dcorr_per_model11 <- ggplot(count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[11] & count_kendall_dcorr_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_per_model.df[which(count_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[11] & count_kendall_dcorr_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_per_model11
bar_kendall_dcorr_per_model11 <- ggplotGrob(bar_kendall_dcorr_per_model11)

f11 <- ggplot(kendall_dcorr_per_model.df[which(kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[11]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_per_model.df[which(sign_kendall_dcorr_per_model.df$source_id == cmip6_data.df$source_id[11]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt11 <- gtable_add_grob(gt11, bar_kendall_dcorr_per_model11, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt11)

plots <- grid.arrange(gt1, gt2, gt3, gt4, gt5, gt6, gt7, gt8, gt9, gt10, gt11, dbar, nrow = 3)


ggsave("testdir/SFig7.png", plot = plots, width = 13/.66/.9, height = 11/.9, units = "in")
...
plots <- grid.arrange(plot_mean_kendall_dcorr, plot_mean_dcorr, plot_mean_dcorr_netrad, plot_first_yr_trans_zone, ncol = 2)

ggsave("testdir/Fig2.png", plot = plots, width = 18, height = 14, units = "in")
...
dbar_trans_zone <- plot_discrete_cbar(myvalues_trans_zone,
                                      colors = c(cols_trans_zone[1],cols_trans_zone[10:12],cols_trans_zone[2:9]),
                                      legend_title = expression("First decade of transition (ELI"[R[n]]*")"),
                                      spacing = "constant",
                                      font_size = 6,
                                      spacing_scaling = 2,
                                      width = .2,
...
plots <- grid.arrange(plot_mean_kendall_dcorr_netrad, plot_first_yr_trans_zone_netrad)


ggsave("testdir/SFig12.png", plot = plots, width = 9, height = 14, units = "in")
