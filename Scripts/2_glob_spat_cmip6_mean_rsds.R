...
# load('RData/202112_dcorr_cmip6_10yr_no_evspsblveg_hurs_rsds.RData')
...
# load("testdir/202208_dcorr_cmip6_10yr_correct_rsds.RData")
...
dcorr_rsds.list <- list(dcorr_rsds.list[[9]], dcorr_rsds.list[[8]], dcorr_rsds.list[[10]], dcorr_rsds.list[[2]],
                          dcorr_rsds.list[[4]], dcorr_rsds.list[[6]])
...
test <- array(NaN,c(180,90,6*12)); test[1:90,,] <- dcorr_rsds.array[91:180,,]; test[91:180,,] <- dcorr_rsds.array[1:90,,]; dcorr_rsds.array <- test; dcorr_rsds.array <- -1*dcorr_rsds.array
  ...
  for(x in 1:180){
    for(y in 1:90){
      if(sum(!is.na(dcorr_all.array[x,y,(i:(i+11))])) == 12){ # only calculate if the model has full time series
        ...
        kendall_dcorr_rsds[x,y,count_i,] <- c(unname(kendallTrendTest(dcorr_rsds.array[x,y,(i:(i+11))])$estimate[2]), unname(kendallTrendTest(dcorr_rsds.array[x,y,(i:(i+11))])$p.value))
      }
    }
  }
  print(i)
...
mmmean_dcorr_per_10yr <- mmmean_dcorr_rsds_per_10yr <- mmmean_tas_per_10yr <- 
    ...
    for(y in 1:90){
      if(!is.na(models_with_full_timeseries[x,y])){
        if(models_with_full_timeseries[x,y] > 4){
          ...
          mmmean_dcorr_rsds_per_10yr[x,y,i] <- mean(dcorr_rsds.array[x,y,(index_per_10yr+i)],na.rm=T)
...
mmmean_kendall_dcorr <-mmmean_kendall_dcorr_rsds <- mmmean_kendall_tas <- mmmean_kendall_hfls <-
...
for(x in 1:180){
  for(y in 1:90){
    if(sum(!is.na(mmmean_dcorr_per_10yr[x,y,])) == 12){
      ...
      mmmean_kendall_dcorr_rsds[x,y,] <- c(unname(kendallTrendTest(mmmean_dcorr_rsds_per_10yr[x,y,])$estimate[2]), unname(kendallTrendTest(mmmean_dcorr_rsds_per_10yr[x,y,])$p.value))
...
for(i in 1:5){
  dcorr_treeFrac.df <- rbind(dcorr_treeFrac.df, 
                             data.frame("mmmean_kendall_dcorr" = c(weighted.mean(x = mask_treeFrac[,,i]*mmmean_kendall_dcorr[,,1], w = area.array*mask_treeFrac[,,i], na.rm = T)),
                                        "mmmean_kendall_dcorr_rsds" = c(weighted.mean(x = mask_treeFrac[,,i]*mmmean_kendall_dcorr_rsds[,,1], w = area.array*mask_treeFrac[,,i], na.rm = T)),
                                        "label" = q_classes[i]))
}
...
bar_treeFractrends_rsds_rev <- ggplot(dcorr_treeFrac.df, aes(y = label, x = mmmean_kendall_dcorr_rsds, fill = label)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "dodge") +
...
bar_treeFractrends_rsds_rev
bar_treeFractrends_rsds_rev <- ggplotGrob(bar_treeFractrends_rsds_rev)
...
# load('testdir/total_land_area.RData')
# load("testdir/202106_cmip6_frac.RData")
# load("testdir/202208_dcorr_cmip6_10yr_correct_netrad.RData")
load("testdir/202208_dcorr_cmip6_10yr_SWin.RData")
...
tas_or_snr.array <- array(NaN,c(180,90,11))
for(i in 1:11){
  for(x in 1:180){
    for(y in 1:90){
      if(sum(!is.na(corr_rgy_veg.list[[i]][x,y,])) > 0){
        if(mean(corr_rgy_veg.list[[i]][x,y,], na.rm = T) > mean(corr_rgy_veg_rsds.list[[i]][x,y,], na.rm = T)){
          ...
          tas_or_snr.array[x,y,i] <- 1 # temperature
        }else{
          tas_or_snr.array[x,y,i] <- 2 # net radiation
        }
      }
    }
  }
...
for(i in 1:132){
  wtr_pos_rgy_neg[i] <- sum(area.array[which(!is.na(dcorr_rsds.array[,,i]) & dcorr_rsds.array[,,i] > 0 & corr_wtr_veg_all.array[,,i] > 0 & corr_rgy_veg_rsds.array[,,i] < 0)], na.rm = T)/sum(area.array[which(!is.na(dcorr_rsds.array[,,i]) & dcorr_rsds.array[,,i] > 0)], na.rm = T)
  wtr_lim_both_pos[i] <- sum(area.array[which(!is.na(dcorr_rsds.array[,,i]) & dcorr_rsds.array[,,i] > 0 & corr_wtr_veg_all.array[,,i] > 0 & corr_rgy_veg_rsds.array[,,i] > 0)], na.rm = T)/sum(area.array[which(!is.na(dcorr_rsds.array[,,i]) & dcorr_rsds.array[,,i] > 0)], na.rm = T)
  wtr_neg_rgy_pos[i] <- sum(area.array[which(!is.na(dcorr_rsds.array[,,i]) & dcorr_rsds.array[,,i] < 0 & corr_wtr_veg_all.array[,,i] < 0 & corr_rgy_veg_rsds.array[,,i] > 0)], na.rm = T)/sum(area.array[which(!is.na(dcorr_rsds.array[,,i]) & dcorr_rsds.array[,,i] < 0)], na.rm = T)
  rgy_lim_both_pos[i] <- sum(area.array[which(!is.na(dcorr_rsds.array[,,i]) & dcorr_rsds.array[,,i] < 0 & corr_wtr_veg_all.array[,,i] > 0 & corr_rgy_veg_rsds.array[,,i] > 0)], na.rm = T)/sum(area.array[which(!is.na(dcorr_rsds.array[,,i]) & dcorr_rsds.array[,,i] < 0)], na.rm = T)
}
  ...
  for(y in 1:90){
    if(!is.na(models_with_full_timeseries[x,y])){
      if(models_with_full_timeseries[x,y] > 4){
        ...
        ensemble_mean_dcorr_rsds[x,y] <- mean(dcorr_rsds.array[x,y,], na.rm = T)
      }
    }
...
for(x in 1:180){
  for(y in 1:90){
    ...
    ensemble_mean_dcorr_rsds_vec[count] <- ensemble_mean_dcorr_rsds[x,y]
...
dcorr.df <- data.frame("ensemble_mean_dcorr_all" = ensemble_mean_dcorr_all_vec,
                       "ensemble_mean_dcorr_rsds" = ensemble_mean_dcorr_rsds_vec,
...
dcorr.df$cuts_ensemble_mean_dcorr_rsds <- cut(dcorr.df$ensemble_mean_dcorr_rsds, myvalues_ensemble_mean_dcorr, include.lowest = T)
...
mmmean_dcorr_per_10yr <- mmmean_corr_rgy_veg_per_10yr <- mmmean_corr_wtr_veg_per_10yr <- mmmean_tas_per_10yr <- mmmean_rsds_per_10yr <- 
  mmmean_dcorr_rsds_per_10yr <- mmmean_corr_rgy_veg_rsds_per_10yr <- 
  mmmean_mrso_per_10yr <- mmmean_hfls_per_10yr <- 
  mmmean_lai_per_10yr <- mmmean_ar_per_10yr <- 
  array(NaN,c(180,90,12))
    ...
    for(y in 1:90){
      if(!is.na(models_with_full_timeseries[x,y])){
        if(models_with_full_timeseries[x,y] > 4){
          ...
          mmmean_dcorr_rsds_per_10yr[x,y,i] <- mean(dcorr_rsds.array[x,y,(index_per_10yr+i)],na.rm=T)
          mmmean_corr_rgy_veg_rsds_per_10yr[x,y,i] <- mean(corr_rgy_veg_rsds.array[x,y,(index_per_10yr+i)],na.rm=T)
          mmmean_corr_wtr_veg_per_10yr[x,y,i] <- mean(corr_wtr_veg_all.array[x,y,(index_per_10yr+i)],na.rm=T)
          mmmean_tas_per_10yr[x,y,i] <- mean(av_tas.array[x,y,(index_per_10yr+i)],na.rm=T)
          mmmean_rsds_per_10yr[x,y,i] <- mean(av_rnet.array[x,y,(index_per_10yr+i)],na.rm=T)
          mmmean_mrso_per_10yr[x,y,i] <- mean(av_mrso.array[x,y,(index_per_10yr+i)],na.rm=T)
          mmmean_hfls_per_10yr[x,y,i] <- mean(av_hfls.array[x,y,(index_per_10yr+i)],na.rm=T)
          mmmean_lai_per_10yr[x,y,i] <- mean(av_lai.array[x,y,(index_per_10yr+i)],na.rm=T)
          mmmean_ar_per_10yr[x,y,i] <- mean(av_ar.array[x,y,(index_per_10yr+i)],na.rm=T)
...
for(i in 1:12){
  area_per_10yr.df <- rbind(area_per_10yr.df,
                            data.frame("year" = year_id[i],
                                       ...
                                       "area_rsds" = 100*c(sum(area.array[which(mmmean_dcorr_rsds_per_10yr[,,i] > 0)])/total_land_area),
                                       "reg" = "Global"))
}
  ...
  for(i in 1:12){
    area_per_10yr.df <- rbind(area_per_10yr.df,
                              data.frame("year" = year_id[i],
                                         ...
                                         "area_rsds" = 100*(sum(area.array[which(lon >= lonmin & lon <= lonmax),
                                                                             which(lat >= latmin & lat <= latmax)][which(mmmean_dcorr_rsds_per_10yr[,,i][which(lon >= lonmin & lon <= lonmax),
                                                                                                                                                           which(lat >= latmin & lat <= latmax)] > 0)])/total_land_area_reg),
                                         "reg" = reg[count_j]))
  }
  count_j <- count_j + 1
...
point_mmmean_dcorr_per_10yr_reg <- ggplot(area_per_10yr.df, aes(x = year, y = area_rsds, col = reg)) +
  geom_line(size = 1) +
  scale_x_continuous("",
                     breaks = year_id[seq(1,11,2)], expand = c(0,0)) +
  scale_y_continuous(expression(paste("water-limited land area-%")), expand = c(0,0)) +
...
for(i in 1:12){
  area_per_10yr.df <- rbind(area_per_10yr.df,
                            data.frame("year" = rep(year_id[i],2),
                                       "count" = c(length(which(mmmean_dcorr_rsds_per_10yr[,,i] > 0)), length(which(mmmean_dcorr_rsds_per_10yr[,,i] < 0))),
                                       "area" = 100*c(sum(area.array[which(mmmean_dcorr_rsds_per_10yr[,,i] > 0)])/total_land_area, sum(area.array[which(mmmean_dcorr_rsds_per_10yr[,,i] < 0)])/total_land_area),
                                       "evap_regime" = c("water-lim.","energy-lim.")))
}
a <- ggplot(dcorr.df[which(!is.na(dcorr.df$ensemble_mean_dcorr_rsds)),], aes(x=lon,y=lat,fill=cuts_ensemble_mean_dcorr_rsds)) + 
  geom_tile() +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id), 
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  # scale_fill_manual(values = c(dcorrcol)) + # watch how many unique col classes there are in dcorr.df$cuts
  scale_fill_manual(values = c(dcorrcol[1:4],dcorrcol[6],dcorrcol[7:9])) + # watch how many unique col classes there are in dcorr.df$cuts
  scale_x_continuous("",
                     limits=c(-180,180),
                     expand=c(0,0)) +
  scale_y_continuous("",
...
point_mmmean_dcorr_rsds_per_10yr <- ggplot(area_per_10yr.df[which(area_per_10yr.df$evap_regime == 'water-lim.'),], aes(x = year, y = area)) +
  geom_line(col = dcorrcol[9], size = 1) + 
  scale_x_continuous("",
                     breaks = year_id[seq(1,11,2)], expand = c(0,0)) +
  scale_y_continuous(expression(paste("water-limited land area-%")), expand = c(0,0)) +
...
point_mmmean_dcorr_rsds_per_10yr
point_mmmean_dcorr_rsds_per_10yr <- ggplotGrob(point_mmmean_dcorr_rsds_per_10yr)
...
plot <- grid.arrange(a,dbar_smaller, nrow = 2, heights = c(.8,.2))
plot_mean_dcorr_rsds <- plot

# plot standard deviation between individual slopes
sd_kendall_dcorr_rsds <- array(NaN,c(180,90))
for(x in 1:180){
  for(y in 1:90){
    if(models_with_full_timeseries[x,y] > 4){
      sd_kendall_dcorr_rsds[x,y] <- sd(kendall_dcorr_rsds[x,y,,1], na.rm = T)
    }
  }
}

mmmean_kendall_dcorr <- mmmean_kendall_corr_rgy_veg <- mmmean_kendall_corr_wtr_veg <- 
  mmmean_kendall_dcorr_rsds <- mmmean_kendall_corr_rgy_veg_rsds <-
  mmmean_kendall_tas <- mmmean_kendall_rsds <- mmmean_kendall_hfls <-
  mmmean_kendall_mrso <- mmmean_kendall_lai <- mmmean_kendall_ar <- 
  array(NaN,c(180,90,2)) # 2: slope (all) & p.value (all)
sum_blocks <- array(NaN,c(180,90))
for(x in 1:180){
  for(y in 1:90){
    if(sum(!is.na(mmmean_dcorr_rsds_per_10yr[x,y,])) == 12){ # full time series
      sum_blocks[x,y] <- sum(!is.na(mmmean_dcorr_per_10yr[x,y,]))
      mmmean_kendall_dcorr[x,y,] <- c(unname(kendallTrendTest(mmmean_dcorr_per_10yr[x,y,])$estimate[2]), unname(kendallTrendTest(mmmean_dcorr_per_10yr[x,y,])$p.value))
      ...
      mmmean_kendall_dcorr_rsds[x,y,] <- c(unname(kendallTrendTest(mmmean_dcorr_rsds_per_10yr[x,y,])$estimate[2]), unname(kendallTrendTest(mmmean_dcorr_rsds_per_10yr[x,y,])$p.value))
      mmmean_kendall_corr_rgy_veg_rsds[x,y,] <- c(unname(kendallTrendTest(mmmean_corr_rgy_veg_rsds_per_10yr[x,y,])$estimate[2]), unname(kendallTrendTest(mmmean_corr_rgy_veg_rsds_per_10yr[x,y,])$p.value))
      mmmean_kendall_tas[x,y,] <- c(unname(kendallTrendTest(mmmean_tas_per_10yr[x,y,])$estimate[2]), unname(kendallTrendTest(mmmean_tas_per_10yr[x,y,])$p.value))
...
sum(area.array[which(mmmean_kendall_dcorr_rsds[,,1] > 0)])/total_land_area # increasing water limitation with incoming shortwave radiation
...
mmmean_kendall_dcorr_vec <- 
  ...
  mmmean_kendall_dcorr_rsds_vec <- 
  mmmean_sign_kendall_dcorr_rsds_vec <- 
  mmmean_kendall_corr_rgy_veg_rsds_vec <- 
  mmmean_sign_kendall_corr_rgy_veg_rsds_vec <- 
  mmmean_kendall_corr_wtr_veg_vec <- 
  mmmean_sign_kendall_corr_wtr_veg_vec <- 
  mmmean_kendall_tas_vec <- 
  mmmean_sign_kendall_tas_vec <- 
  mmmean_kendall_rsds_vec <- 
  mmmean_sign_kendall_rsds_vec <- 
...
for(x in 1:180){
  for(y in 1:90){
    ...
    mmmean_kendall_dcorr_rsds_vec[count] <- mmmean_kendall_dcorr_rsds[x,y,1]
    mmmean_sign_kendall_dcorr_rsds_vec[count] <- mmmean_kendall_dcorr_rsds[x,y,2]
    mmmean_kendall_corr_rgy_veg_rsds_vec[count] <- mmmean_kendall_corr_rgy_veg_rsds[x,y,1]
    mmmean_sign_kendall_corr_rgy_veg_rsds_vec[count] <- mmmean_kendall_corr_rgy_veg_rsds[x,y,2]
    ...
    mmmean_kendall_rsds_vec[count] <- mmmean_kendall_rsds[x,y,1]
    mmmean_sign_kendall_rsds_vec[count] <- mmmean_kendall_rsds[x,y,2]
    ...
    sd_kendall_dcorr_rsds_vec[count] <- sd_kendall_dcorr_rsds[x,y]
    count <- count + 1
  }
}
...
mmtrends.df <- data.frame('trend_dcorr' = c(mmmean_kendall_dcorr_vec),
                          ...
                          'trend_dcorr_rsds' = c(mmmean_kendall_dcorr_rsds_vec),
                          'trend_corr_rgy_veg_rsds' = c(mmmean_kendall_corr_rgy_veg_rsds_vec),
                          'trend_corr_wtr_veg' = c(mmmean_kendall_corr_wtr_veg_vec),
                          'trend_tas' = c(mmmean_kendall_tas_vec),
                          'trend_rsds' = c(mmmean_kendall_rsds_vec),
                          ...
                          'sd_kendall_dcorr_rsds' = c(sd_kendall_dcorr_rsds_vec),
                          "lon" = rep(lon, each = 90),
                          "lat" = rep(lat, 180))
...
sign_dcorr_rsds.df <- data.frame("lon" = rep(lon, each = 90)[which(mmmean_sign_kendall_dcorr_rsds_vec < .05)],
                                   "lat" = rep(lat, 180)[which(mmmean_sign_kendall_dcorr_rsds_vec < .05)])
sign_corr_rgy_veg_rsds.df <- data.frame("lon" = rep(lon, each = 90)[which(mmmean_sign_kendall_corr_rgy_veg_rsds_vec < .05)],
                                          "lat" = rep(lat, 180)[which(mmmean_sign_kendall_corr_rgy_veg_rsds_vec < .05)])
...
sign_rsds.df <- data.frame("lon" = rep(lon, each = 90)[which(mmmean_sign_kendall_rsds_vec < .05)],
                             "lat" = rep(lat, 180)[which(mmmean_sign_kendall_rsds_vec < .05)])
...
dbar_dcorr_rsds <- plot_discrete_cbar(myvalues_trnd_dcorr,
                                        colors = cols_trnd_dcorr,
...
dbar_corr_rgy_veg <- plot_discrete_cbar(myvalues_trnd_corr_rgy_veg,
                                        ...
                                        legend_title = expression("cor(SW"["in"]*"',ET') trend (-/10yr)"),
                                        spacing = "constant",
                                        font_size = 6,
                                        spacing_scaling = 2,
                                        width = .2,
...
cols_trnd_rsds <- rev(brewer.pal(9,"RdYlBu")[1:7])
myvalues_trnd_rsds <- c(-Inf,seq(-.02,.08,.02),Inf)
dbar_rsds <- plot_discrete_cbar(myvalues_trnd_rsds,
                                  colors = cols_trnd_rsds,
                                  legend_title = expression(paste("Shortwave inc. radiation trend (mm/10yr)")),
                                  spacing = "constant",
                                  font_size = 6,
                                  spacing_scaling = 2,
                                  width = .2,
...
mmtrends.df$cuts_trend_dcorr_rsds <- cut(mmtrends.df$trend_dcorr_rsds, myvalues_trnd_dcorr, include.lowest = T)
mmtrends.df$cuts_trend_corr_rgy_veg_rsds <- cut(mmtrends.df$trend_corr_rgy_veg_rsds, myvalues_trnd_corr_rgy_veg, include.lowest = T)
...
mmtrends.df$cuts_trend_rsds <- cut(mmtrends.df$trend_rsds, myvalues_trnd_rsds, include.lowest = T)
...
mmtrends.df$cuts_sd_kendall_dcorr_rsds <- cut(mmtrends.df$sd_kendall_dcorr_rsds, myvalues_trnd_sd_dcorr, include.lowest = T)
...
count_mmtrends.df <- data.frame("area" = c(100*c(sum(area.array[which(mmmean_kendall_dcorr_rsds[,,1] > 0)])/total_land_area, sum(area.array[which(mmmean_kendall_dcorr_rsds[,,1] < 0)])/total_land_area),
                                           100*c(sum(area.array[which(mmmean_kendall_dcorr_rsds[,,1] > 0 & mmmean_kendall_dcorr_rsds[,,2] < .05)])/total_land_area, sum(area.array[which(mmmean_kendall_dcorr_rsds[,,1] < 0 & mmmean_kendall_dcorr_rsds[,,2] < .05)])/total_land_area),
                                           100*c(sum(area.array[which(mmmean_kendall_dcorr_rsds[,,1] > 0 & mmmean_kendall_dcorr_rsds[,,2] > .05)])/total_land_area, sum(area.array[which(mmmean_kendall_dcorr_rsds[,,1] < 0 & mmmean_kendall_dcorr_rsds[,,2] > .05)])/total_land_area)),
                                "trend" = rep(c("drying","wettening"),3),
                                "sign" = rep(c("all","sign","insign"),each=2))
count_mmtrends.df$trend_sign <- paste0(count_mmtrends.df$trend, "_",count_mmtrends.df$sign)

f <- ggplot(mmtrends.df[which(!is.na(mmtrends.df$trend_dcorr_rsds)),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) + 
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_dcorr_rsds.df, aes(x=lon,y=lat), size = .05) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id), 
           inherit.aes = F, fill = NA, color = "black", size = .1) +
...
gt1 <- gtable(widths = unit(rep(2,32), c("null")), heights = unit(rep(2,32), "null"))
gt1 <- gtable_add_grob(gt1, f, t=1, b=32, l=1, r=32)
gt1 <- gtable_add_grob(gt1, bar_treeFractrends_rsds_rev, t = 30, l = 3, b = 19, r = 10)
gt1 <- gtable_add_grob(gt1, bar_mmtrends, t = 21, l = 3, b = 13, r = 8)
grid.draw(gt1)

# reduce top and bottom margins
empty <- ggplot() + theme_void()
dbar_dcorr_rsds <- dbar_dcorr_rsds + theme(plot.margin = unit(c(-35, 10, -30, 10), "pt"))
dbar_smaller <- grid.arrange(empty, dbar_dcorr_rsds, empty , ncol=3, widths = c(1,4,1))

plot <- grid.arrange(gt1,dbar_smaller, nrow = 2, heights = c(.8,.2))
plot_mean_kendall_dcorr_rsds <- plot
...
count_mmtrends.df <- data.frame("area" = c(100*c(sum(area.array[which(mmmean_kendall_rsds[,,1] > 0)])/total_land_area, sum(area.array[which(mmmean_kendall_rsds[,,1] < 0)])/total_land_area),
                                           100*c(sum(area.array[which(mmmean_kendall_rsds[,,1] > 0 & mmmean_kendall_rsds[,,2] < .05)])/total_land_area, sum(area.array[which(mmmean_kendall_rsds[,,1] < 0 & mmmean_kendall_rsds[,,2] < .05)])/total_land_area),
                                           100*c(sum(area.array[which(mmmean_kendall_rsds[,,1] > 0 & mmmean_kendall_rsds[,,2] > .05)])/total_land_area, sum(area.array[which(mmmean_kendall_rsds[,,1] < 0 & mmmean_kendall_rsds[,,2] > .05)])/total_land_area)),
                                "trend" = rep(c("warming","cooling"),3),
                                "sign" = rep(c("all","sign","insign"),each=2))
count_mmtrends.df$trend_sign <- paste0(count_mmtrends.df$trend, "_",count_mmtrends.df$sign)

f <- ggplot(mmtrends.df[which(!is.na(mmtrends.df$trend_rsds)),], aes(x=lon,y=lat,fill=cuts_trend_rsds)) + 
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_rsds.df, aes(x=lon,y=lat), size = .05) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id), 
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_rsds,
                    drop = F) +
  scale_x_continuous("",
                     limits=c(-180,180),
                     expand=c(0,0)) +
...
bar_mmtrends <- ggplot(count_mmtrends.df[which(count_mmtrends.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  ...
  geom_text(inherit.aes = F, data = count_mmtrends.df[which(count_mmtrends.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 2.5) + 
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("warming_sign" = cols_trnd_rsds[7],
                               "warming_insign" = alpha(cols_trnd_rsds[7], .25),
                               "cooling_insign" = alpha(cols_trnd_rsds[1], .25),
                               "cooling_sign" = cols_trnd_rsds[1]),
                    drop = F) +
  theme(legend.position = "none",
        legend.text = element_text(size=12),
        legend.title = element_text(size=20),
...
# reduce top and bottom margins
empty <- ggplot() + theme_void()
dbar_rsds <- dbar_rsds + theme(plot.margin = unit(c(-35, 10, -30, 10), "pt"))
dbar_smaller <- grid.arrange(empty, dbar_rsds, empty , ncol=3, widths = c(1,4,1))

plot <- grid.arrange(gt1,dbar_smaller, nrow = 2, heights = c(.8,.2))
plot_mean_kendall_rsds <- plot
...
plots1 <- grid.arrange(plot_mean_kendall_rsds, plot_mean_kendall_mrso, plot_mean_kendall_hfls, ncol = 3, widths = c(1/3,1/3,1/3))
...
count_mmtrends.df <- data.frame("area" = c(100*c(sum(area.array[which(mmmean_kendall_corr_rgy_veg_rsds[,,1] > 0)])/total_land_area, sum(area.array[which(mmmean_kendall_corr_rgy_veg_rsds[,,1] < 0)])/total_land_area),
                                           100*c(sum(area.array[which(mmmean_kendall_corr_rgy_veg_rsds[,,1] > 0 & mmmean_kendall_corr_rgy_veg_rsds[,,2] < .05)])/total_land_area, sum(area.array[which(mmmean_kendall_corr_rgy_veg_rsds[,,1] < 0 & mmmean_kendall_corr_rgy_veg_rsds[,,2] < .05)])/total_land_area),
                                           100*c(sum(area.array[which(mmmean_kendall_corr_rgy_veg_rsds[,,1] > 0 & mmmean_kendall_corr_rgy_veg_rsds[,,2] > .05)])/total_land_area, sum(area.array[which(mmmean_kendall_corr_rgy_veg_rsds[,,1] < 0 & mmmean_kendall_corr_rgy_veg_rsds[,,2] > .05)])/total_land_area)),
                                "trend" = rep(c("+ energy lim.","- energy lim."),3),
                                "sign" = rep(c("all","sign","insign"),each=2))
count_mmtrends.df$trend_sign <- paste0(count_mmtrends.df$trend, "_",count_mmtrends.df$sign)

f <- ggplot(mmtrends.df[which(!is.na(mmtrends.df$trend_corr_rgy_veg)),], aes(x=lon,y=lat,fill=cuts_trend_corr_rgy_veg_rsds)) + 
  geom_tile() +
...
# the mean sd_kendall_dcorr_rsds plot
f <- ggplot(mmtrends.df[which(!is.na(mmtrends.df$trend_dcorr_rsds)),], aes(x=lon,y=lat,fill=cuts_sd_kendall_dcorr_rsds)) +
  geom_tile() +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
...
for(x in 1:180){
  for(y in 1:90){
    if(!is.na(models_with_full_timeseries[x,y])){
      if(models_with_full_timeseries[x,y] > 4){
        agreement.array[x,y] <- length(which(sign(kendall_dcorr_rsds[x,y,,1]) == sign(mmmean_kendall_dcorr_rsds[x,y,1]))) / sum(!is.na(kendall_dcorr_rsds[x,y,,1]))*100
      }
    }
  }
}
...
# make spatial plots per model for the dcorr_rsds trend
kendall_dcorr_rsds_per_model.df <- setNames(data.frame(matrix(ncol = 4, nrow = 0)),
                                              c("trend_dcorr_rsds","lon","lat","source_id"))
for(i in 1:11){
  for(x in 1:180){
    for(y in 1:90){
      if(!is.na(kendall_dcorr_rsds[x,y,i,1])){
        kendall_dcorr_rsds_per_model.df <- rbind(kendall_dcorr_rsds_per_model.df,
                                                   data.frame("trend_dcorr_rsds" = kendall_dcorr_rsds[x,y,i,1],
                                                              "p" = kendall_dcorr_rsds[x,y,i,2],
                                                              "lon" = lon[x],
                                                              "lat" = lat[y],
...
kendall_dcorr_rsds_per_model.df$cuts_trend_dcorr_rsds <- cut(kendall_dcorr_rsds_per_model.df$trend_dcorr_rsds, myvalues_trnd_dcorr, include.lowest = T)

sign_kendall_dcorr_rsds_per_model.df <- kendall_dcorr_rsds_per_model.df[,c(3,4,5)][which(kendall_dcorr_rsds_per_model.df$p < .05),]

# calculate total_land_area_sourceid
total_land_area_sourceid <- c()
for(i in 1:11){
  total_land_area_sourceid[i] <- sum(area.array[which(!is.na(kendall_dcorr_rsds[,,i,1]))])
}



# the dcorr_rsds trend per source_id
count_kendall_dcorr_rsds_per_model.df <- data.frame("area" = c(100*c(sum(area.array[which(kendall_dcorr_rsds[,,1,1] > 0)])/total_land_area_sourceid[1], sum(area.array[which(kendall_dcorr_rsds[,,1,1] < 0)])/total_land_area_sourceid[1],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,1,1] > 0 & kendall_dcorr_rsds[,,1,2] < .05)])/total_land_area_sourceid[1], sum(area.array[which(kendall_dcorr_rsds[,,1,1] < 0 & kendall_dcorr_rsds[,,1,2] < .05)])/total_land_area_sourceid[1],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,1,1] > 0 & kendall_dcorr_rsds[,,1,2] > .05)])/total_land_area_sourceid[1], sum(area.array[which(kendall_dcorr_rsds[,,1,1] < 0 & kendall_dcorr_rsds[,,1,2] > .05)])/total_land_area_sourceid[1]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,2,1] > 0)])/total_land_area_sourceid[2], sum(area.array[which(kendall_dcorr_rsds[,,2,1] < 0)])/total_land_area_sourceid[2],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,2,1] > 0 & kendall_dcorr_rsds[,,2,2] < .05)])/total_land_area_sourceid[2], sum(area.array[which(kendall_dcorr_rsds[,,2,1] < 0 & kendall_dcorr_rsds[,,2,2] < .05)])/total_land_area_sourceid[2],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,2,1] > 0 & kendall_dcorr_rsds[,,2,2] > .05)])/total_land_area_sourceid[2], sum(area.array[which(kendall_dcorr_rsds[,,2,1] < 0 & kendall_dcorr_rsds[,,2,2] > .05)])/total_land_area_sourceid[2]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,3,1] > 0)])/total_land_area_sourceid[3], sum(area.array[which(kendall_dcorr_rsds[,,3,1] < 0)])/total_land_area_sourceid[3],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,3,1] > 0 & kendall_dcorr_rsds[,,3,2] < .05)])/total_land_area_sourceid[3], sum(area.array[which(kendall_dcorr_rsds[,,3,1] < 0 & kendall_dcorr_rsds[,,3,2] < .05)])/total_land_area_sourceid[3],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,3,1] > 0 & kendall_dcorr_rsds[,,3,2] > .05)])/total_land_area_sourceid[3], sum(area.array[which(kendall_dcorr_rsds[,,3,1] < 0 & kendall_dcorr_rsds[,,3,2] > .05)])/total_land_area_sourceid[3]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,4,1] > 0)])/total_land_area_sourceid[4], sum(area.array[which(kendall_dcorr_rsds[,,4,1] < 0)])/total_land_area_sourceid[4],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,4,1] > 0 & kendall_dcorr_rsds[,,4,2] < .05)])/total_land_area_sourceid[4], sum(area.array[which(kendall_dcorr_rsds[,,4,1] < 0 & kendall_dcorr_rsds[,,4,2] < .05)])/total_land_area_sourceid[4],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,4,1] > 0 & kendall_dcorr_rsds[,,4,2] > .05)])/total_land_area_sourceid[4], sum(area.array[which(kendall_dcorr_rsds[,,4,1] < 0 & kendall_dcorr_rsds[,,4,2] > .05)])/total_land_area_sourceid[4]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,5,1] > 0)])/total_land_area_sourceid[5], sum(area.array[which(kendall_dcorr_rsds[,,5,1] < 0)])/total_land_area_sourceid[5],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,5,1] > 0 & kendall_dcorr_rsds[,,5,2] < .05)])/total_land_area_sourceid[5], sum(area.array[which(kendall_dcorr_rsds[,,5,1] < 0 & kendall_dcorr_rsds[,,5,2] < .05)])/total_land_area_sourceid[5],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,5,1] > 0 & kendall_dcorr_rsds[,,5,2] > .05)])/total_land_area_sourceid[5], sum(area.array[which(kendall_dcorr_rsds[,,5,1] < 0 & kendall_dcorr_rsds[,,5,2] > .05)])/total_land_area_sourceid[5]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,6,1] > 0)])/total_land_area_sourceid[6], sum(area.array[which(kendall_dcorr_rsds[,,6,1] < 0)])/total_land_area_sourceid[6],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,6,1] > 0 & kendall_dcorr_rsds[,,6,2] < .05)])/total_land_area_sourceid[6], sum(area.array[which(kendall_dcorr_rsds[,,6,1] < 0 & kendall_dcorr_rsds[,,6,2] < .05)])/total_land_area_sourceid[6],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,6,1] > 0 & kendall_dcorr_rsds[,,6,2] > .05)])/total_land_area_sourceid[6], sum(area.array[which(kendall_dcorr_rsds[,,6,1] < 0 & kendall_dcorr_rsds[,,6,2] > .05)])/total_land_area_sourceid[6]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,7,1] > 0)])/total_land_area_sourceid[7], sum(area.array[which(kendall_dcorr_rsds[,,7,1] < 0)])/total_land_area_sourceid[7],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,7,1] > 0 & kendall_dcorr_rsds[,,7,2] < .05)])/total_land_area_sourceid[7], sum(area.array[which(kendall_dcorr_rsds[,,7,1] < 0 & kendall_dcorr_rsds[,,7,2] < .05)])/total_land_area_sourceid[7],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,7,1] > 0 & kendall_dcorr_rsds[,,7,2] > .05)])/total_land_area_sourceid[7], sum(area.array[which(kendall_dcorr_rsds[,,7,1] < 0 & kendall_dcorr_rsds[,,7,2] > .05)])/total_land_area_sourceid[7]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,8,1] > 0)])/total_land_area_sourceid[8], sum(area.array[which(kendall_dcorr_rsds[,,8,1] < 0)])/total_land_area_sourceid[8],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,8,1] > 0 & kendall_dcorr_rsds[,,8,2] < .05)])/total_land_area_sourceid[8], sum(area.array[which(kendall_dcorr_rsds[,,8,1] < 0 & kendall_dcorr_rsds[,,8,2] < .05)])/total_land_area_sourceid[8],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,8,1] > 0 & kendall_dcorr_rsds[,,8,2] > .05)])/total_land_area_sourceid[8], sum(area.array[which(kendall_dcorr_rsds[,,8,1] < 0 & kendall_dcorr_rsds[,,8,2] > .05)])/total_land_area_sourceid[8]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,9,1] > 0)])/total_land_area_sourceid[9], sum(area.array[which(kendall_dcorr_rsds[,,9,1] < 0)])/total_land_area_sourceid[9],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,9,1] > 0 & kendall_dcorr_rsds[,,9,2] < .05)])/total_land_area_sourceid[9], sum(area.array[which(kendall_dcorr_rsds[,,9,1] < 0 & kendall_dcorr_rsds[,,9,2] < .05)])/total_land_area_sourceid[9],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,9,1] > 0 & kendall_dcorr_rsds[,,9,2] > .05)])/total_land_area_sourceid[9], sum(area.array[which(kendall_dcorr_rsds[,,9,1] < 0 & kendall_dcorr_rsds[,,9,2] > .05)])/total_land_area_sourceid[9]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,10,1] > 0)])/total_land_area_sourceid[10], sum(area.array[which(kendall_dcorr_rsds[,,10,1] < 0)])/total_land_area_sourceid[10],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,10,1] > 0 & kendall_dcorr_rsds[,,10,2] < .05)])/total_land_area_sourceid[10], sum(area.array[which(kendall_dcorr_rsds[,,10,1] < 0 & kendall_dcorr_rsds[,,10,2] < .05)])/total_land_area_sourceid[10],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,10,1] > 0 & kendall_dcorr_rsds[,,10,2] > .05)])/total_land_area_sourceid[10], sum(area.array[which(kendall_dcorr_rsds[,,10,1] < 0 & kendall_dcorr_rsds[,,10,2] > .05)])/total_land_area_sourceid[10]),
                                                                 100*c(sum(area.array[which(kendall_dcorr_rsds[,,11,1] > 0)])/total_land_area_sourceid[11], sum(area.array[which(kendall_dcorr_rsds[,,11,1] < 0)])/total_land_area_sourceid[11],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,11,1] > 0 & kendall_dcorr_rsds[,,11,2] < .05)])/total_land_area_sourceid[11], sum(area.array[which(kendall_dcorr_rsds[,,11,1] < 0 & kendall_dcorr_rsds[,,11,2] < .05)])/total_land_area_sourceid[11],
                                                                       sum(area.array[which(kendall_dcorr_rsds[,,11,1] > 0 & kendall_dcorr_rsds[,,11,2] > .05)])/total_land_area_sourceid[11], sum(area.array[which(kendall_dcorr_rsds[,,11,1] < 0 & kendall_dcorr_rsds[,,11,2] > .05)])/total_land_area_sourceid[11])),
...
count_kendall_dcorr_rsds_per_model.df$trend_sign <- paste0(count_kendall_dcorr_rsds_per_model.df$trend, "_",count_kendall_dcorr_rsds_per_model.df$sign)

bar_kendall_dcorr_rsds_per_model1 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[1] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[1] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model1
bar_kendall_dcorr_rsds_per_model1 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model1)

f1 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[1]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[1]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt1 <- gtable_add_grob(gt1, bar_kendall_dcorr_rsds_per_model1, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt1)


bar_kendall_dcorr_rsds_per_model2 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[2] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[2] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model2
bar_kendall_dcorr_rsds_per_model2 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model2)

f2 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[2]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[2]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt2 <- gtable_add_grob(gt2, bar_kendall_dcorr_rsds_per_model2, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt2)

bar_kendall_dcorr_rsds_per_model3 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[3] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[3] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model3
bar_kendall_dcorr_rsds_per_model3 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model3)

f3 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[3]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[3]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt3 <- gtable_add_grob(gt3, bar_kendall_dcorr_rsds_per_model3, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt3)

bar_kendall_dcorr_rsds_per_model4 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[4] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[4] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model4
bar_kendall_dcorr_rsds_per_model4 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model4)

f4 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[4]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[4]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt4 <- gtable_add_grob(gt4, bar_kendall_dcorr_rsds_per_model4, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt4)

bar_kendall_dcorr_rsds_per_model5 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[5] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[5] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model5
bar_kendall_dcorr_rsds_per_model5 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model5)

f5 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[5]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[5]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt5 <- gtable_add_grob(gt5, bar_kendall_dcorr_rsds_per_model5, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt5)

bar_kendall_dcorr_rsds_per_model6 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[6] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[6] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model6
bar_kendall_dcorr_rsds_per_model6 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model6)

f6 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[6]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[6]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt6 <- gtable_add_grob(gt6, bar_kendall_dcorr_rsds_per_model6, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt6)

bar_kendall_dcorr_rsds_per_model7 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[7] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[7] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model7
bar_kendall_dcorr_rsds_per_model7 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model7)

f7 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[7]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[7]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt7 <- gtable_add_grob(gt7, bar_kendall_dcorr_rsds_per_model7, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt7)

bar_kendall_dcorr_rsds_per_model8 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[8] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[8] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model8
bar_kendall_dcorr_rsds_per_model8 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model8)

f8 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[8]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[8]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt8 <- gtable_add_grob(gt8, bar_kendall_dcorr_rsds_per_model8, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt8)

bar_kendall_dcorr_rsds_per_model9 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[9] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[9] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model9
bar_kendall_dcorr_rsds_per_model9 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model9)

f9 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[9]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[9]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt9 <- gtable_add_grob(gt9, bar_kendall_dcorr_rsds_per_model9, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt9)

bar_kendall_dcorr_rsds_per_model10 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[10] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[10] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model10
bar_kendall_dcorr_rsds_per_model10 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model10)

f10 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[10]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[10]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt10 <- gtable_add_grob(gt10, bar_kendall_dcorr_rsds_per_model10, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt10)

bar_kendall_dcorr_rsds_per_model11 <- ggplot(count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[11] & count_kendall_dcorr_rsds_per_model.df$sign != 'all'),], aes(x = trend, y = area, group = trend, fill = trend_sign)) +
  geom_bar(stat='identity',col = 'black', width = .5, position = "stack") +
  geom_text(inherit.aes = F, data = count_kendall_dcorr_rsds_per_model.df[which(count_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[11] & count_kendall_dcorr_rsds_per_model.df$sign == 'all'),], aes(x=trend,y=area-5,label=round(area,2)), size = 1.5) + 
  scale_x_discrete("") +
  scale_y_continuous(expression(paste("land area-%")), expand = c(0,0)) +
  scale_fill_manual(values = c("drying_sign" = brewer.pal(9,"Reds")[7],
                               "drying_insign" = alpha(brewer.pal(9,"Reds")[7], .25),
...
bar_kendall_dcorr_rsds_per_model11
bar_kendall_dcorr_rsds_per_model11 <- ggplotGrob(bar_kendall_dcorr_rsds_per_model11)

f11 <- ggplot(kendall_dcorr_rsds_per_model.df[which(kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[11]),], aes(x=lon,y=lat,fill=cuts_trend_dcorr_rsds)) +
  geom_tile() +
  geom_point(inherit.aes = F, data = sign_kendall_dcorr_rsds_per_model.df[which(sign_kendall_dcorr_rsds_per_model.df$source_id == cmip6_data.df$source_id[11]),], aes(x=lon,y=lat), size = .005) +
  geom_map(data = mymap, map = mymap, aes(x = long, y = lat, map_id = id),
           inherit.aes = F, fill = NA, color = "black", size = .1) +
  geom_segment(data = hotspot_regs.df, inherit.aes = F, aes(x = x, y = y, xend = xend, yend = yend), lty = 'dashed') +
  scale_fill_manual(values = cols_trnd_dcorr,
...
gt11 <- gtable_add_grob(gt11, bar_kendall_dcorr_rsds_per_model11, t = 31, l = 5, b = 18, r = 11)
grid.draw(gt11)

plots <- grid.arrange(gt1, gt2, gt3, gt4, gt5, gt6, gt7, gt8, gt9, gt10, gt11, dbar_dcorr_rsds, nrow = 3)
...
for(i in 2:12){
  for(x in 1:180){
    for(y in 1:90){
      if(sum(!is.na(mmmean_dcorr_rsds_per_10yr[x,y,((i-1):i)])) == 2){
        if(sign(mmmean_dcorr_rsds_per_10yr[x,y,(i-1)]) != sign(mmmean_dcorr_rsds_per_10yr[x,y,(i)])){
          trans_zone[x,y,i] <- year_id[i-1]
        }
      }
    }
...
gt1 <- gtable(widths = unit(rep(2,32), c("null")), heights = unit(rep(2,32), "null"))
gt1 <- gtable_add_grob(gt1, c, t=1, b=32, l=1, r=32)
gt1 <- gtable_add_grob(gt1, point_mmmean_dcorr_rsds_per_10yr, t = 30, l = 4, b = 22, r = 10)
grid.draw(gt1)
...
dbar_trans_zone <- dbar_trans_zone + theme(plot.margin = unit(c(-35, 10, -30, 10), "pt"))
plot <- grid.arrange(gt1,dbar_trans_zone, nrow = 2, heights = c(.8,.2))
plot_first_yr_trans_zone_rsds <- plot
...
plots <- grid.arrange(plot_mean_kendall_dcorr_rsds, plot_mean_dcorr_rsds, plot_mean_dcorr, plot_first_yr_trans_zone_rsds, ncol = 2)
