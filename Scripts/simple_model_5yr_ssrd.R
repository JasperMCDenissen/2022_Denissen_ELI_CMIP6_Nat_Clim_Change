...
#     
#     # MJ/m2
#     path_ssrd <- "/Net/Groups/data_BGC/era5/e1/0d50_daily/ssrd/"
#     ssrd <- c()
#     count <- 1
#     for(year in (1981+t):(1981+t+4)){
#       ssrd_ncin <- nc_open(paste0(path_ssrd,'ssrd.daily.fc.era5.720.360.',year,'.nc'))
#       if(year %% 4 != 0){
#         ssrd <- c(ssrd, ncvar_get(ssrd_ncin, 'ssrd', start = c(lons[xy],lats[xy],1), count = c(1,1,365))/86400*10^6/26.15741)
#       }else{
#         ssrd <- c(ssrd, ncvar_get(ssrd_ncin, 'ssrd', start = c(lons[xy],lats[xy],1), count = c(1,1,366))/86400*10^6/26.15741)
#       }
#       nc_close(ssrd_ncin)
#       print(paste0("ssrd in year ",year," is done..."))
#     }
# 
...
#       corr_rgy_veg[xy,count_t] <- cor(anom_ssrd_mon[which(t2m_mon > 283.15)],
#                                       anom_ET_mon[which(t2m_mon > 283.15)],
#                                       method = 'kendall', use = "pairwise.complete.obs")
...
# 
# save(comp.df,
#      file = "testdir/202204_simple_model_ELI_5yr_ssrd.RData")
# from /RData
load('RData/202204_simple_model_ELI_5yr_ssrd.RData')
# from /testdir
