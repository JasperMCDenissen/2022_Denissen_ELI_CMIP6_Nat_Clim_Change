...
dcorr_all <- dcorr_all.array; av_tas <- av_tas.array; av_mrso <- av_mrso.array; av_hfls <- av_hfls.array; av_lai <- av_lai.array; av_ar <- av_ar.array
count <- 1
for(t in seq(1,(11*12),12)){
  for(x in 1:180){
    for(y in 1:90){
      if(sum(!is.na(dcorr_all[x,y,(t:(t+11))])) == 12 & sum(!is.na(av_lai[x,y,(t:(t+11))])) == 12 & round(sd(av_lai[x,y,(t:(t+11))]),4) != 0 & round(sd(av_mrso[x,y,(t:(t+11))]),4) != 0){ # there should be variability in mrso and lai, otherwise there is no explanatory power in their respective time series
        relimp_variables.list <- list()
        rsq.vec <- c()
        # dredge: automated model selection
        lm_dcorr.df <- lm(dcorr_all[x,y,(t:(t+11))] ~
                            av_tas[x,y,(t:(t+11))] + av_mrso[x,y,(t:(t+11))] + av_hfls[x,y,(t:(t+11))] +
...
bar_dom_var_dredge_relimp <- ggplot(count_dom_var_dredge_relimp.df, aes(x = var_factor, y = area, fill = var_factor)) +
  ...
  scale_fill_manual("",
                    values = cols_dom_var,
                    labels = c("Temperature","Soil moisture","Terrestrial evaporation","Leaf Area Index","Aridity Index"),
                    drop = F) +
...
ggsave("testdir/SFig15.png", plot = bar_dom_var_dredge_relimp, width = 9, height = 7, units = "in")
