...
dpcorr.list <- list(dpcorr.list[[1]], dpcorr.list[[2]], dpcorr.list[[3]], dpcorr.list[[4]],
                   dpcorr.list[[5]], dpcorr.list[[6]], dpcorr.list[[7]], dpcorr.list[[9]],
                   dpcorr.list[[10]], dpcorr.list[[11]], dpcorr.list[[12]])
...
pcorr_rgy_veg.list <- list(pcorr_rgy_veg.list[[1]], pcorr_rgy_veg.list[[2]], pcorr_rgy_veg.list[[3]], pcorr_rgy_veg.list[[4]],
                   pcorr_rgy_veg.list[[5]], pcorr_rgy_veg.list[[6]], pcorr_rgy_veg.list[[7]], pcorr_rgy_veg.list[[9]],
                   pcorr_rgy_veg.list[[10]], pcorr_rgy_veg.list[[11]], pcorr_rgy_veg.list[[12]])
...
pcorr_wtr_veg.list <- list(pcorr_wtr_veg.list[[1]], pcorr_wtr_veg.list[[2]], pcorr_wtr_veg.list[[3]], pcorr_wtr_veg.list[[4]],
                           pcorr_wtr_veg.list[[5]], pcorr_wtr_veg.list[[6]], pcorr_wtr_veg.list[[7]], pcorr_wtr_veg.list[[9]],
                           pcorr_wtr_veg.list[[10]], pcorr_wtr_veg.list[[11]], pcorr_wtr_veg.list[[12]])
...
# cmip6_data.df <- data.frame("source_id" = c(cmip6_data.df$source_id[1:7], cmip6_data.df$source_id[9:12]),
#                             "member_id" = c(cmip6_data.df$member_id[1:7], cmip6_data.df$member_id[9:12]))
...
tseries.df <- setNames(data.frame(matrix(ncol = 16, nrow = 0)),
                       c("dcorr", "corr_rgy_veg", "corr_wtr_veg",
                         "dcorr_netrad", "corr_rgy_veg_netrad", "corr_wtr_veg_netrad",
                         "dpcorr", "pcorr_rgy_veg", "pcorr_wtr_veg",
                         "tas","mrso","hfls","lai","ar","pr","netrad"))
for(i in 1:132){
  tseries.df <- rbind(tseries.df, 
                      data.frame("dcorr" = weighted.mean(x = mask_obs*dcorr_all.array[,,i], w = area.array*mask_obs, na.rm = T),
                                 ...
                                 "dcorr_netrad" = weighted.mean(x = mask_obs*dcorr_netrad.array[,,i], w = area.array*mask_obs, na.rm = T),
                                 "corr_rgy_veg_netrad" = weighted.mean(x = mask_obs*corr_rgy_veg_netrad.array[,,i], w = area.array*mask_obs, na.rm = T),
                                 "corr_wtr_veg_netrad" = weighted.mean(x = mask_obs*corr_wtr_veg_netrad.array[,,i], w = area.array*mask_obs, na.rm = T),
                                 ...
                                 "netrad" = weighted.mean(x = mask_obs*av_rnet.array[,,i], w = area.array*mask_obs, na.rm = T)
                                 ))
}
...
tseries.df$dcorr_abs_1980 <-
  ...
  tseries.df$dcorr_netrad_abs_1980 <-
  tseries.df$corr_rgy_veg_netrad_abs_1980 <-
  tseries.df$corr_wtr_veg_netrad_abs_1980 <-
...
for(i in 1:132){
  ...
  tseries.df$dcorr_netrad_abs_1980[i] <- (tseries.df$dcorr_netrad[i] - tseries.df$dcorr_netrad[which(tseries.df$source_id == tseries.df$source_id[i])][1])
  tseries.df$corr_rgy_veg_netrad_abs_1980[i] <- (tseries.df$corr_rgy_veg_netrad[i] - tseries.df$corr_rgy_veg_netrad[which(tseries.df$source_id == tseries.df$source_id[i])][1])
  tseries.df$corr_wtr_veg_netrad_abs_1980[i] <- (tseries.df$corr_wtr_veg_netrad[i] - tseries.df$corr_wtr_veg_netrad[which(tseries.df$source_id == tseries.df$source_id[i])][1])
  ...
  tseries.df$netrad_abs_1980[i] <- (tseries.df$netrad[i] - tseries.df$netrad[which(tseries.df$source_id == tseries.df$source_id[i])][1])
}
...
for(i in 1:12){
  ...
  mmmtseries.df$mmmedian_dcorr_netrad[i] <- median(tseries.df$dcorr_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$perc25_dcorr_netrad[i] <- quantile(tseries.df$dcorr_netrad_abs_1980[which(mmindex == i)], prob = c(.25), na.rm = T)
  mmmtseries.df$perc75_dcorr_netrad[i] <- quantile(tseries.df$dcorr_netrad_abs_1980[which(mmindex == i)], prob = c(.75), na.rm = T)
  mmmtseries.df$mmmean_dcorr_netrad[i] <- mean(tseries.df$dcorr_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$mmmean_minsd_dcorr_netrad[i] <- mmmtseries.df$mmmean_dcorr_netrad[i] - sd(tseries.df$dcorr_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$mmmean_plussd_dcorr_netrad[i] <- mmmtseries.df$mmmean_dcorr_netrad[i] + sd(tseries.df$dcorr_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  ...
  mmmtseries.df$mmmedian_corr_rgy_veg_netrad[i] <- median(tseries.df$corr_rgy_veg_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$perc25_corr_rgy_veg_netrad[i] <- quantile(tseries.df$corr_rgy_veg_netrad_abs_1980[which(mmindex == i)], prob = c(.25), na.rm = T)
  mmmtseries.df$perc75_corr_rgy_veg_netrad[i] <- quantile(tseries.df$corr_rgy_veg_netrad_abs_1980[which(mmindex == i)], prob = c(.75), na.rm = T)
  mmmtseries.df$mmmean_corr_rgy_veg_netrad[i] <- mean(tseries.df$corr_rgy_veg_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$mmmean_minsd_corr_rgy_veg_netrad[i] <- mmmtseries.df$mmmean_corr_rgy_veg_netrad[i] - sd(tseries.df$corr_rgy_veg_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$mmmean_plussd_corr_rgy_veg_netrad[i] <- mmmtseries.df$mmmean_corr_rgy_veg_netrad[i] + sd(tseries.df$corr_rgy_veg_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  ...
  mmmtseries.df$mmmedian_corr_wtr_veg_netrad[i] <- median(tseries.df$corr_wtr_veg_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$perc25_corr_wtr_veg_netrad[i] <- quantile(tseries.df$corr_wtr_veg_netrad_abs_1980[which(mmindex == i)], prob = c(.25), na.rm = T)
  mmmtseries.df$perc75_corr_wtr_veg_netrad[i] <- quantile(tseries.df$corr_wtr_veg_netrad_abs_1980[which(mmindex == i)], prob = c(.75), na.rm = T)
  mmmtseries.df$mmmean_corr_wtr_veg_netrad[i] <- mean(tseries.df$corr_wtr_veg_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$mmmean_minsd_corr_wtr_veg_netrad[i] <- mmmtseries.df$mmmean_corr_wtr_veg_netrad[i] - sd(tseries.df$corr_wtr_veg_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$mmmean_plussd_corr_wtr_veg_netrad[i] <- mmmtseries.df$mmmean_corr_wtr_veg_netrad[i] + sd(tseries.df$corr_wtr_veg_netrad_abs_1980[which(mmindex == i)], na.rm = T)
  ...
  mmmtseries.df$mmmedian_netrad[i] <- median(tseries.df$netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$perc25_netrad[i] <- quantile(tseries.df$netrad_abs_1980[which(mmindex == i)], prob = c(.25), na.rm = T)
  mmmtseries.df$perc75_netrad[i] <- quantile(tseries.df$netrad_abs_1980[which(mmindex == i)], prob = c(.75), na.rm = T)
  mmmtseries.df$mmmean_netrad[i] <- mean(tseries.df$netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$mmmean_minsd_netrad[i] <- mmmtseries.df$mmmean_netrad[i] - sd(tseries.df$netrad_abs_1980[which(mmindex == i)], na.rm = T)
  mmmtseries.df$mmmean_plussd_netrad[i] <- mmmtseries.df$mmmean_netrad[i] + sd(tseries.df$netrad_abs_1980[which(mmindex == i)], na.rm = T)
}
...
mmmtseries.df_norm <- data.frame("median_norm" = c(mmmtseries.df$mmmedian_dcorr / sd(mmmtseries.df$mmmedian_dcorr - c(1:12)*kendallTrendTest(mmmtseries.df$mmmedian_dcorr)$estimate[2]),
                                                   ...
                                                   mmmtseries.df$mmmedian_dcorr_netrad / sd(mmmtseries.df$mmmedian_dcorr_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmedian_dcorr_netrad)$estimate[2]),
                                                   mmmtseries.df$mmmedian_corr_rgy_veg_netrad / sd(mmmtseries.df$mmmedian_corr_rgy_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmedian_corr_rgy_veg_netrad)$estimate[2]),
                                                   mmmtseries.df$mmmedian_corr_wtr_veg_netrad / sd(mmmtseries.df$mmmedian_corr_wtr_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmedian_corr_wtr_veg_netrad)$estimate[2]),
                                 ...
                                 "perc25_norm" = c(mmmtseries.df$perc25_dcorr / sd(mmmtseries.df$mmmedian_dcorr - c(1:12)*kendallTrendTest(mmmtseries.df$mmmedian_dcorr)$estimate[2]),
                                                   ...
                                                   mmmtseries.df$perc25_dcorr_netrad / sd(mmmtseries.df$mmmedian_dcorr_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmedian_dcorr_netrad)$estimate[2]),
                                                   mmmtseries.df$perc25_corr_rgy_veg_netrad / sd(mmmtseries.df$perc25_corr_rgy_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$perc25_corr_rgy_veg_netrad)$estimate[2]),
                                                   mmmtseries.df$perc25_corr_wtr_veg_netrad / sd(mmmtseries.df$perc25_corr_wtr_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$perc25_corr_wtr_veg_netrad)$estimate[2]),
                                 ...
                                 "perc75_norm" = c(mmmtseries.df$perc75_dcorr / sd(mmmtseries.df$mmmedian_dcorr - c(1:12)*kendallTrendTest(mmmtseries.df$mmmedian_dcorr)$estimate[2]),
                                                   ...
                                                   mmmtseries.df$perc75_dcorr_netrad / sd(mmmtseries.df$mmmedian_dcorr_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmedian_dcorr_netrad)$estimate[2]),
                                                   mmmtseries.df$perc75_corr_rgy_veg_netrad / sd(mmmtseries.df$perc75_corr_rgy_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$perc75_corr_rgy_veg_netrad)$estimate[2]),
                                                   mmmtseries.df$perc75_corr_wtr_veg_netrad / sd(mmmtseries.df$perc75_corr_wtr_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$perc75_corr_wtr_veg_netrad)$estimate[2]),
                                 ...
                                 "mean_norm" = c(mmmtseries.df$mmmean_dcorr / sd(mmmtseries.df$mmmean_dcorr - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_dcorr)$estimate[2]),
                                                 ...
                                                 mmmtseries.df$mmmean_dcorr_netrad / sd(mmmtseries.df$mmmean_dcorr_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_dcorr_netrad)$estimate[2]),
                                                 mmmtseries.df$mmmean_corr_rgy_veg_netrad / sd(mmmtseries.df$mmmean_corr_rgy_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_corr_rgy_veg_netrad)$estimate[2]),
                                                 mmmtseries.df$mmmean_corr_wtr_veg_netrad / sd(mmmtseries.df$mmmean_corr_wtr_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_corr_wtr_veg_netrad)$estimate[2]),
                                 ...
                                 "mean_minsd_norm" = c(mmmtseries.df$mmmean_minsd_dcorr / sd(mmmtseries.df$mmmean_dcorr - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_dcorr)$estimate[2]),
                                                       ...
                                                       mmmtseries.df$mmmean_minsd_dcorr_netrad / sd(mmmtseries.df$mmmean_dcorr_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_dcorr_netrad)$estimate[2]),
                                                       mmmtseries.df$mmmean_minsd_corr_rgy_veg_netrad / sd(mmmtseries.df$mmmean_minsd_corr_rgy_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_minsd_corr_rgy_veg_netrad)$estimate[2]),
                                                       mmmtseries.df$mmmean_minsd_corr_wtr_veg_netrad / sd(mmmtseries.df$mmmean_minsd_corr_wtr_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_minsd_corr_wtr_veg_netrad)$estimate[2]),
                                 ...
                                 "mean_plussd_norm" = c(mmmtseries.df$mmmean_plussd_dcorr / sd(mmmtseries.df$mmmean_dcorr - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_dcorr)$estimate[2]),
                                                        ...
                                                        mmmtseries.df$mmmean_plussd_dcorr_netrad / sd(mmmtseries.df$mmmean_dcorr_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_dcorr_netrad)$estimate[2]),
                                                        mmmtseries.df$mmmean_plussd_corr_rgy_veg_netrad / sd(mmmtseries.df$mmmean_plussd_corr_rgy_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_plussd_corr_rgy_veg_netrad)$estimate[2]),
                                                        mmmtseries.df$mmmean_plussd_corr_wtr_veg_netrad / sd(mmmtseries.df$mmmean_plussd_corr_wtr_veg_netrad - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_plussd_corr_wtr_veg_netrad)$estimate[2]),
                                                        ...
                                                        mmmtseries.df$mmmean_plussd_hfls / sd(mmmtseries.df$mmmean_hfls - c(1:12)*kendallTrendTest(mmmtseries.df$mmmean_hfls)$estimate[2])),
                                 "var" = c(rep("dcorr",12),rep("corr_rgy_veg",12),rep("corr_wtr_veg",12),rep("dcorr_netrad",12),rep("corr_rgy_veg_netrad",12),rep("corr_wtr_veg_netrad",12),rep("dpcorr",12),rep("pcorr_rgy_veg",12),rep("pcorr_wtr_veg",12),rep("tas",12),rep("mrso",12),rep("hfls",12),rep("lai",12),rep("ar",12),rep("pr",12),rep("netrad",12)),
...
mmmtseries.df_norm$var_factor <- factor(mmmtseries.df_norm$var, levels = c("dcorr","corr_wtr_veg","corr_rgy_veg","tas","mrso","hfls","lai","ar","pr"))
for_the_plot.df <- mmmtseries.df_norm[which(mmmtseries.df_norm$var != "pr" & mmmtseries.df_norm$var != "dpcorr" &
                                              ...
                                              mmmtseries.df_norm$var != "corr_rgy_veg_netrad" & mmmtseries.df_norm$var != "corr_wtr_veg_netrad" &
                                              mmmtseries.df_norm$var != "dcorr_netrad" & mmmtseries.df_norm$var != "netrad"),]
# abs 1980
m4 <- ggplot(for_the_plot.df, aes(x=year,y=mean_norm,col=var_factor,fill=var_factor,linetype=var_factor,shape=var_factor)) +
  ...
  scale_color_manual("", values = cols_var, labels = c("Ecosystem Limitation Index (ELI)","cor(SM',ET')",expression("cor(T"[a]*"',ET')"),expression("Temperature (T"[a]*")"),"Soil moisture (SM)", "Terrestrial evaporation (ET)", "Leaf Area Index", "Aridity Index")) +
  scale_fill_manual("", values = cols_var, labels = c("Ecosystem Limitation Index (ELI)","cor(SM',ET')",expression("cor(T"[a]*"',ET')"),expression("Temperature (T"[a]*")"),"Soil moisture (SM)", "Terrestrial evaporation (ET)", "Leaf Area Index", "Aridity Index")) +
  scale_linetype_manual("", values = c(rep(1,3),2:6), labels = c("Ecosystem Limitation Index (ELI)","cor(SM',ET')",expression("cor(T"[a]*"',ET')"),expression("Temperature (T"[a]*")"),"Soil moisture (SM)", "Terrestrial evaporation (ET)", "Leaf Area Index", "Aridity Index")) +
  scale_shape_manual("", values = c(16,15,17,rep(16,5)), labels = c("Ecosystem Limitation Index (ELI)","cor(SM',ET')",expression("cor(T"[a]*"',ET')"),expression("Temperature (T"[a]*")"),"Soil moisture (SM)", "Terrestrial evaporation (ET)", "Leaf Area Index", "Aridity Index")) +
...
m4 <- ggplot(tseries.df, aes(x=year,y=dcorr_abs_1980,group=source_id)) +
  ...
  scale_x_continuous("",
                     breaks = seq(1980,2090,20),
                     labels = seq(1980,2090,20), expand = c(0,0)) +
  scale_y_continuous(expression(paste("change since 1980")), expand = c(0,0), limits = c(-.03,.18)) +
...
m4_rgy <- ggplot(tseries.df, aes(x=year,y=corr_rgy_veg_abs_1980,group=source_id)) + 
  ...
  ) + ggtitle(expression("cor(T"[a]*"',ET')"))
m4_rgy

n4 <- ggplot(tseries.df, aes(x=year,y=tas_abs_1980,group=source_id)) + 
  geom_line(linetype = 'dashed', col = cols_var[2]) +
  geom_hline(yintercept=0) +
  geom_point(inherit.aes = F, data = mmmtseries.df, aes(x=year,y=mmmean_tas), col = cols_var[2], size = 2.5) +
  geom_line(inherit.aes = F, data = mmmtseries.df, aes(x=year,y=mmmean_tas), col = cols_var[2], linetype = 'solid', size = 1.2) +
  geom_ribbon(inherit.aes = F, data = mmmtseries.df, aes(x=year,ymin=perc25_tas,ymax=perc75_tas), col = cols_var[2], fill = cols_var[2], alpha = 0.5) +
  scale_x_continuous("", 
                     breaks = seq(1980,2090,30),
                     labels = seq(1980,2090,30), expand = c(0,0)) +
  ...
  ggtitle(expression("b) Temperature (T"[a]*")"))
n4
...
ggsave("testdir/Fig1.png", plot = plots, width = 10*1.6, height = 10*1.6, units = "in")

m4 <- ggplot(tseries.df, aes(x=year,y=dcorr_netrad_abs_1980,group=source_id)) +
  geom_line(linetype = 'dashed', col = cols_var[1]) +
  geom_point(inherit.aes = F, data = mmmtseries.df, aes(x=year,y=mmmean_dcorr_netrad), col = cols_var[1], size = 2.5) +
  geom_line(inherit.aes = F, data = mmmtseries.df, aes(x=year,y=mmmean_dcorr_netrad), col = cols_var[1], linetype = 'solid', size = 1.2) +
  geom_ribbon(inherit.aes = F, data = mmmtseries.df, aes(x=year,ymin=mmmean_minsd_dcorr_netrad,ymax=mmmean_plussd_dcorr_netrad), col = cols_var[1], fill = cols_var[1], alpha = 0.5) +
  geom_hline(yintercept=0) +
  scale_x_continuous("",
                     breaks = seq(1980,2090,20),
                     labels = seq(1980,2090,20), expand = c(0,0)) +
...
m4_rgy <- ggplot(tseries.df, aes(x=year,y=corr_rgy_veg_netrad_abs_1980,group=source_id)) + 
  geom_line(linetype = 'dashed', col = brewer.pal(n=9,'BrBG')[8]) +
  geom_hline(yintercept=0) +
  geom_point(inherit.aes = F, data = mmmtseries.df, aes(x=year,y=mmmean_corr_rgy_veg_netrad), col = brewer.pal(n=9,'BrBG')[8], size = 2.5) +
  geom_line(inherit.aes = F, data = mmmtseries.df, aes(x=year,y=mmmean_corr_rgy_veg_netrad), col = brewer.pal(n=9,'BrBG')[8], linetype = 'solid', size = 1.2) +
  geom_ribbon(inherit.aes = F, data = mmmtseries.df, aes(x=year,ymin=perc25_corr_rgy_veg_netrad,ymax=perc75_corr_rgy_veg_netrad), col = brewer.pal(n=9,'BrBG')[8], fill = brewer.pal(n=9,'BrBG')[8], alpha = 0.5) +
  scale_x_continuous("", 
                     breaks = seq(1980,2090,30),
                     labels = seq(1980,2090,30), expand = c(0,0)) +
  scale_y_continuous(expression(paste("change since 1980")), expand = c(0,0), limits = c(-.23,.03)) +
  theme(legend.position = "none",
        legend.text = element_text(size=12),
        legend.title = element_text(size=20),
        legend.key.width = unit(1.5, "cm"),
  ...
  ) + ggtitle(expression("b) cor(R"[n]*"',ET')"))
m4_rgy

plot_netrad <- plot_grid(m4, m4_rgy, ncol = 2)

ggsave("testdir/SFig1.png", plot = plot_netrad, width = 10*1.25, height = 5*1.25, units = "in")

mmmtseries_pcorr.df <- data.frame("value" = c(mmmtseries.df$mmmean_dcorr, mmmtseries.df$mmmean_dpcorr,
                                            mmmtseries.df$mmmean_corr_rgy_veg, mmmtseries.df$mmmean_pcorr_rgy_veg,
                                            mmmtseries.df$mmmean_corr_wtr_veg, mmmtseries.df$mmmean_pcorr_wtr_veg),
...
m4 <- ggplot(mmmtseries_pcorr.df, aes(x=year,y=value,col=var_factor,linetype=var_factor)) +
  ...
  scale_color_manual("variable", 
                     values = cols_pcorr,
                     labels = c("ELI (cor)", "ELI (pcor)", expression("cor(T"[a]*"',ET')"), expression("cor(T"[a]*"',ET'|SM')"), "cor(SM',ET')", expression("cor(SM',ET'|T"[a]*"')"))) +
...
# # /testdir
# load('testdir/202112_dcorr_cmip6_10yr.RData')
...
av_netrad.array <- av_rnet.array[,,c(1:12,13:24,37:48,49:60,61:72,85:96,109:120)]
...
# Calculate area-weighted average dcorr,tas,mrso,hfls
tseries.df <- setNames(data.frame(matrix(ncol = 14, nrow = 0)),
                       c("dcorr","corr_rgy_veg", "corr_wtr_veg", "dcorr_t", "corr_rgy_veg_t", "corr_wtr_veg_t", 
                         "hurs","VPD","evspsblveg", "hfls", "T_ET", "pr","netrad","baresoilevap"))
for(i in 1:84){
  tseries.df <- rbind(tseries.df, 
                      data.frame("dcorr" = weighted.mean(x = mask_obs*dcorr_all.array[,,i], w = area.array*mask_obs, na.rm = T),
                                 "corr_rgy_veg" = weighted.mean(x = mask_obs*corr_rgy_veg_all.array[,,i], w = area.array*mask_obs, na.rm = T),
                                 ...
                                 "pr" = weighted.mean(x = mask_obs*av_pr.array[,,i], w = area.array*mask_obs, na.rm = T),
                                 "netrad" = weighted.mean(x = mask_obs*av_netrad.array[,,i], w = area.array*mask_obs, na.rm = T),
...
tseries.df$dcorr_abs_1980 <-
  ...
  tseries.df$pr_abs_1980 <- 
  tseries.df$netrad_abs_1980 <- 
  tseries.df$baresoilevap_abs_1980 <- NaN
...
w4 <- ggplot(tseries.df, aes(x=year,y=netrad_abs_1980,col=source_id)) + 
  geom_hline(yintercept=0) +
  geom_point(inherit.aes = F, data = mmmtseries.df, aes(x=year,y=mmmean_netrad), col = 1, size = 1.5) +
  geom_line(inherit.aes = F, data = mmmtseries.df, aes(x=year,y=mmmean_netrad), col = 1, linetype = 'solid') +
  geom_ribbon(inherit.aes = F, data = mmmtseries.df, aes(x=year,ymin=mmmean_minsd_netrad,ymax=mmmean_plussd_netrad), col = brewer.pal(n=9,'Greys')[5], alpha = 0.5) +
  scale_x_continuous("", 
                     breaks = seq(1980,2090,30),
                     labels = seq(1980,2090,30), expand = c(0,0)) +
  scale_y_continuous(expression(paste("change since 1980 (mm/d)")), expand = c(0,0)) +
  ...
  ggtitle("c) Net surface radiation")
w4

plots <- plot_grid(s4, t4, w4, v4, rel_heights = c(1/2,1/2), align = 'v')

ggsave("testdir/SFig2.png", plot = plots, width = 10*1.5, height = 10*1.5, units = "in")
...
m4 <- ggplot(mmmtseries_t.df, aes(x=year,y=value,col=var_factor,linetype=var_factor)) +
  ...
  scale_color_manual("variable", 
                     values = cols_t,
                     labels = c(expression("ELI"), expression("ELI"[T]), expression("cor(T"[a]*"',ET')"), expression("cor(T"[a]*"',T')"), "cor(SM',ET')", expression("cor(SM',T')"))) +
  scale_linetype_manual("variable", 
                        values = linetypes_t,
                        labels = c(expression("ELI"), expression("ELI"[T]), expression("cor(T"[a]*"',ET')"), expression("cor(T"[a]*"',T')"), "cor(SM',ET')", expression("cor(SM',T')"))) +
  theme(legend.position = c(.1,.85),
...
m4
ggsave("testdir/SFig4.png", plot = m4, width = 10*1.25, height = 5*1.25, units = "in")
