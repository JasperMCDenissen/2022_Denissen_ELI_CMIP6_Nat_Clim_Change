...
dcorr_rsds_all.array <- av_mrso.array <- array(NaN,c(180,90,11*12)) # 11 different source_id and 12*10 years
count_all <- 1
for(i in 1:11){
...
ensemble_mean_dcorr_rsds_all <- 
  array(NaN,c(180,90))
for(x in 1:180){
  for(y in 1:90){
    if(!is.na(models_with_full_timeseries[x,y])){
      if(models_with_full_timeseries[x,y] > 4){
        ensemble_mean_dcorr_rsds_all[x,y] <- mean(dcorr_rsds_all.array[x,y,], na.rm = T)
      }
    }
  }
}

# first calculate the month-of-year multimodel mean dcorr_rsds
...
for(x in 1:180){ 
  for(y in 1:90){
    for(i in 1:144){ # loop over consequently 
      inter_dcorr_rsds <- c() # to collect models with same month and 10 yr block
      for(source_id in 1:11){
        inter_dcorr_rsds[source_id] <- dcorr_seascycle.list[[source_id]][x,y,i]
      }
      if(sum(!is.na(inter_dcorr_rsds)) > 4){
        mmmean_dcorr_seascycle[x,y,i] <- mean(inter_dcorr_rsds, na.rm = T)
      }
    }
  }
  ...
  for(t in 1:144){ # loop over all moy and blocks
    ...
    if(sum(!is.na(mmmean_dcorr_seascycle[which(lon >= lonmin & lon <= lonmax),
      ...
      dcorr_seascycle_reg.df <- rbind(dcorr_seascycle_reg.df,
                                      data.frame("dcorr_rsds" = weighted.mean(x = mmmean_dcorr_seascycle[which(lon >= lonmin & lon <= lonmax),
...
# dcorr_rsdscol <- brewer.pal(9,"BrBG")
dcorr_rsdscol <- c("[-Inf,-0.66]" = brewer.pal(9,"BrBG")[9],
...
dcorr_seascycle_reg.df$cuts_dcorr_rsds <- cut(dcorr_seascycle_reg.df$dcorr_rsds, myvalues, include.lowest = T)

mmmean_fingerprintNAM <- ggplot(dcorr_seascycle_reg.df[which(dcorr_seascycle_reg.df$reg == reg[2]),], aes(x=center_year,y=moy,fill=cuts_dcorr_rsds)) + 
  geom_tile() + 
  ...
  scale_x_continuous("",breaks = seq(1980,2100,30),
                     expand = c(0,0)) +
  scale_fill_manual(values = dcorr_rsdscol, na.value = 'white') + # watch how many unique col classes there are in dcorr_rsds.df$cuts
...
mmmean_fingerprintSAM <- ggplot(dcorr_seascycle_reg.df[which(dcorr_seascycle_reg.df$reg == reg[1]),], aes(x=center_year,y=moy,fill=cuts_dcorr_rsds)) + 
  geom_tile() + 
  scale_y_continuous("",breaks = seq(1,12),
                     labels = c("Jan","Feb","Mar","Apr","May","Jun",
                                "Jul","Aug","Sep","Oct","Nov","Dec"),
...
mmmean_fingerprintNEA <- ggplot(dcorr_seascycle_reg.df[which(dcorr_seascycle_reg.df$reg == reg[4]),], aes(x=center_year,y=moy,fill=cuts_dcorr_rsds)) + 
  geom_tile() + 
  scale_y_continuous("",breaks = seq(1,12),
                     labels = c("Jan","Feb","Mar","Apr","May","Jun",
                                "Jul","Aug","Sep","Oct","Nov","Dec"),
...
mmmean_fingerprintCEU <- ggplot(dcorr_seascycle_reg.df[which(dcorr_seascycle_reg.df$reg == reg[3]),], aes(x=center_year,y=moy,fill=cuts_dcorr_rsds)) + 
  geom_tile() + 
  scale_y_continuous("",breaks = seq(1,12),
                     labels = c("Jan","Feb","Mar","Apr","May","Jun",
                                "Jul","Aug","Sep","Oct","Nov","Dec"),
...
mmmean_fingerprintEAS <- ggplot(dcorr_seascycle_reg.df[which(dcorr_seascycle_reg.df$reg == reg[5]),], aes(x=center_year,y=moy,fill=cuts_dcorr_rsds)) + 
  geom_tile() + 
  scale_y_continuous("",breaks = seq(1,12),
                     labels = c("Jan","Feb","Mar","Apr","May","Jun",
                                "Jul","Aug","Sep","Oct","Nov","Dec"),
...
# plot the discrete colorbar for mean dcorr_rsds
dbar <- plot_discrete_cbar(breaks = c(myvalues[1:4],0,myvalues[7:10]),
                           colors = c(unname(dcorr_rsdscol[1:4]),unname(dcorr_rsdscol[6:9])),
...
g <- ggplot(moy_trends.df[which(!is.na(moy_trends.df$cuts_trnd_in_wtr_lim_months)),], aes(x=lon,y=lat,fill=cuts_trnd_in_wtr_lim_months_newfac_fac)) +
  ...
  scale_fill_manual("Trend in length of water-limited season (months/120yr)",
                    values = cols_trnd,
                    drop = F) + # watch how many unique col classes there are in dcorr_rsds.df$cuts
...
# plot the discrete colorbar for median dcorr_rsds
