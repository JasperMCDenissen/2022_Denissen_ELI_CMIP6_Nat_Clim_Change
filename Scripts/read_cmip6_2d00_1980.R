...
files_he <- list.files(cmip6_path_he,
                       pattern = ".nc$")
vars <- c('tas','mrso','hurs','evspsblveg')
moy <- rep(1:12,10)
  dcorr_t.list <-
  corr_rgy_veg_t.list <-
  corr_wtr_veg_t.list <-
...
for(i in 1:length(cmip6_data.df$source_id)){
  ...
  for(t in seq(1,1440,120)){ # so the loop moves 10 years and in the loop we read in 10 years of data.
    ...
    stack_anom_tas_evspsblveg_wrm <- stack(anom_tas_wrm_10yr, anom_evspsblveg_wrm_10yr)
    stack_anom_mrso_evspsblveg_wrm <- stack(anom_mrso_wrm_10yr, anom_evspsblveg_wrm_10yr)
    corr_rgy_veg_10yr_t <- clusterR(stack_anom_tas_evspsblveg_wrm, calc, args = list(corr_fun_10yr), progress = 'text')
  ...
  save(dcorr_t.list,
       ...
       av_hurs.list,
       av_evspsblveg.list,
       file = "testdir/202112_dcorr_cmip6_10yr.RData")
}
